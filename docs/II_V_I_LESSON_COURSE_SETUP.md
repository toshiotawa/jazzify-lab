...
1. `env.r2.example` を参考に **`.env.r2` または `env.r2`** に `CF_ACCOUNT_ID`（必須）、必要なら `R2_BUCKET`
2. `npx wrangler login`
3. `npm install` 後、`node scripts/upload-ii-v-i-mp3-to-r2.mjs`（`--dry-run` / `--s3` はスクリプトヘッダー参照）。単発の put は `npm run wrangler:r2 -- r2 object put …`（[FANTASY_STAGE_SETUP_GUIDE.md](./FANTASY_STAGE_SETUP_GUIDE.md#21-ii-v-i-レッスンコース-bgm-一括アップロードcli)）
