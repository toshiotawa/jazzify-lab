...
2. [管理画面でBGMアップロード（R2 + カウントイン付与）](#2-管理画面でbgmアップロードr2--カウントイン付与)
   ...
   - [2.2 II-V-I: カウントインと音源・楽譜の同期](#22-ii-v-i-カウントインと音源楽譜の同期)
...
| 概念 | 意味 |
|---|---|
| **生の再生位置 0 秒** | カウントイン区間の**開始**（メトロノーム・無音など） |
| **`loopBegin` 秒**（= `count_in_measures × (60/BPM) × 拍子`） | 楽譜の **M1・拍1**＝ゲーム内「音楽時間 0」 |
| **楽譜・ノーツ** | MusicXML の小節 1 からが演奏。カウントイン用の空小節は XML に無くてよい（正規化後に M1 が 0 秒に一致） |

そのため **バッキング MP3 がいきなり本編（M1）から始まっている**と、`count_in_measures = 1` のままでは **約 1 小節分**、音とノーツ／楽譜がズレます。

**対処（推奨）:**

1. **音源の先頭に、DB と同じ長さのカウントイン区間を付ける**（無音でも可）。長さは `1 小節 = (60/BPM) × time_signature` 秒（例: BPM 160・4/4 なら **1.5 秒**）。
2. リポジトリ付属スクリプトで一括処理する場合:  
   `node scripts/prepend-count-in-to-ii-v-i-mp3.mjs --dry-run` で対象確認 →  
   **バックアップを取ったうえで** `node scripts/prepend-count-in-to-ii-v-i-mp3.mjs --in-place`  
   （`ffmpeg` が PATH に必要。各ファイルに `.bak` が残る）
3. 上書き後、従来どおり [2.1](#21-ii-v-i-レッスンコース-bgm-一括アップロードcli) の手順で R2 に再アップロードする。

**代替（音源を触らない場合）:** DB の `count_in_measures` を **0** にし、ゲーム側のカウントイン期待を外す。ただし「演奏開始前の 1 小節分の猶予」は音源側に無いので、体験は先頭付与の場合と異なる。

**クリック音が欲しい場合:** 管理画面の「カウント追加」（[上記「カウントイン自動付与」](#カウントイン自動付与の仕組み)）や DAW で先頭にクリックを付けてから R2 にアップロードする。
