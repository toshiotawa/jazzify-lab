...
1. **Supabase SQL**（1 SELECT ずつ）  
   ...
   - [`scripts/analytics/weekly_main_quest_assignments.sql`](../scripts/analytics/weekly_main_quest_assignments.sql) … 第1章クエスト/課題クリア・脱落・MIDI  
...
- ローカル CLI: `jazzify-ga-report/.env` の `GA_PROPERTY_ID`（数字のプロパティ ID）
