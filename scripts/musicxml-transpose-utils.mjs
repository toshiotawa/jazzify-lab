/**
 * MusicXML 移調ユーティリティ（251 / Major II-V-I Bebop Licks 共用）。
 */
import { JSDOM } from 'jsdom';
import { Note, Interval, Key } from 'tonal';

export const TARGET_KEYS_NON_C = [
 ...
 * @returns {string}
 */
export function transposeMusicXmlString(xmlString, semitones, targetKeyName, window) {
  const win = window ?? new JSDOM('').window;
  const { DOMParser } = win;
  ...
  return serializeMusicXml(xmlDoc, win, xmlString);
}
 ...
 * @returns {string}
 */
export function setMusicXmlTempo(xmlString, tempo) {
  return xmlString.replace(/tempo="\d+(\.\d+)?"/g, `tempo="${tempo}"`);
}
