#!/usr/bin/env node
/**
 * II-V-I Short 1Bar コース用マイグレーション SQL 生成。
 ...
 *
 * Usage:
 *   node scripts/generate-major-251-licks-migration.mjs
 */
...
const OUT = join(ROOT, 'supabase', 'migrations', '20260802150000_m251_short_1bar_stage_lessons.sql');
/** 既存 R2 資産の URL を維持する（内容は変わっていないためキャッシュを流用） */
const CACHE_BUST_XML = '202608011920';
const CACHE_BUST_MP3 = '202608011800';
const MISS_DAMAGE = 5;
...
const stageRows = SHORT_1BAR_STAGES.map(
...
const stageSlug = "'m251-s1-st' || s.si || '-' || k.slug || '-' || m.ms";
const lessonSlug = "'m251-s1-lesson-st' || s.si";
const stageTitle = "'Short 1Bar Stage ' || s.si";
...
const sql = `-- ${COURSE_TITLE}: ${SHORT_1BAR_STAGES.length} stages, 1 stage = 1 block = 1 lesson (${KEYS.length} keys x battle/precision)
BEGIN;
...
SELECT
  ${uuidV5(stageSlug)},
  ${stageSlug},
  ${stageTitle} || ' ' || ${songTitleJa},
  ${stageTitle} || ' ' || ${songTitleEn},
  ${descJa},
  ${descEn},
...
SELECT
  ...
  ${songTitleJa},
  ${songTitleEn},
  '${CDN_BASE}/' || ${stageSlug} || '.musicxml?v=${CACHE_BUST_XML}',
  '${CDN_BASE}/m251-s1-st' || s.si || '-' || k.slug || '.mp3?v=${CACHE_BUST_MP3}',
...
console.log(
  `Stages/lessons: ${SHORT_1BAR_STAGES.length}, Songs: ${SHORT_1BAR_STAGES.length * KEYS.length * 2}`,
);
