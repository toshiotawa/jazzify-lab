#!/usr/bin/env node
/**
 * Short II-V 2 Bars 資産を R2 の `sozai/major-251-short-2bars/` にアップロード。
 *
 * Usage:
 *   node scripts/upload-major-251-short-2bars-r2.mjs --s3
 *   node scripts/upload-major-251-short-2bars-r2.mjs --dry-run
 */
...
function expectedFiles() {
  /** @type {string[]} */
  const names = [];
  for (const stage of SHORT_2BARS_STAGES) {
...
function listUploadFiles() {
  ...
  return readdirSync(OUT_DIR).filter(
    (name) => name.startsWith('m251-s2-') && (name.endsWith('.mp3') || name.endsWith('.musicxml')),
  );
}
...
async function main() {
  ...
  if (files.length === 0) {
    console.error(`No files in ${OUT_DIR}. Run prepare-major-251-short-2bars-assets.mjs first.`);
    process.exit(1);
  }
  for (const name of files.sort()) {
    await uploadOne(name);
  }
  console.log(`Done upload-major-251-short-2bars-r2 (${files.length} files)`);
}
