#!/usr/bin/env node
/**
 * II-V-I Short 1Bar アセット生成（1 stage = 1 lesson 分）。
 *
 * Usage:
 *   node scripts/prepare-major-251-licks-assets.mjs
 *   node scripts/prepare-major-251-licks-assets.mjs --xml-only
 *   node scripts/prepare-major-251-licks-assets.mjs --mp3-only
 *   node scripts/prepare-major-251-licks-assets.mjs --stage 1 --key c
 */
...
function selectedStages() {
  return SHORT_1BAR_STAGES.filter((s) => stageOnly === null || s.stageIndex === stageOnly);
}
...
function main() {
  ...
  if (!mp3Only) {
    const statsPath = join(OUT_DIR, 'm251-s1-verify.json');
    if (!dryRun) {
