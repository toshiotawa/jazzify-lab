...
interface SixNoteChordTemplate {
...
interface SixNoteProgressionTemplate {
  ...
  readonly variants: readonly { readonly patternIds: readonly SixNotePatternId[] }[];
}
...
const MAJOR_3_PATTERNS: SixNoteMajorPatternSet = {
  ...
  abc_inversion: ['D5', 'C5', 'G4', 'E4', 'B5', 'A5'],
...
const MINOR6_PATTERN_IDS: readonly SixNotePatternId[] = [
  'abc', 'bca', 'aprime_abc', 'eca', 'abda', 'ada',
];

const MAJOR3_PATTERN_IDS: readonly SixNotePatternId[] = [
...
const buildSingleChordProgression = (
  ...
  chordName: string,
  patterns: SixNotePatternSet | SixNoteMajorPatternSet,
  patternIds: readonly SixNotePatternId[],
...
const buildProgressionTemplateEntries = (
  ...
  for (const key of ALL_MAJOR_KEYS) {
    ...
    for (const variant of template.variants) {
      template.chords.forEach((chord, chordIndex) => {
        const patternId = variant.patternIds[chordIndex];
        if (!patternId) {
          throw new Error(`Missing pattern id for chord index ${chordIndex}`);
        }
        const noteNames = transposeNotes(getPatternNotes(chord.patterns, patternId), interval);
...
const SINGLE_SPECS: readonly {
  slug: string;
  titleJa: string;
  titleEn: string;
  sortOrder: number;
  referenceKey: MajorKey;
  chord: SixNoteChordTemplate;
}[] = [
  {
    ...
    sortOrder: 1,
    referenceKey: 'C',
    chord: { chordName: 'Dm7', patterns: M7_MINOR6_PATTERNS, patternIds: MINOR6_PATTERN_IDS },
  },
  {
    slug: 'six-note-scale-7-m7',
    titleJa: '7(m7)',
    titleEn: '7(m7)',
    sortOrder: 2,
    referenceKey: 'C',
    chord: { chordName: 'G7(Dm7)', patterns: M7_MINOR6_PATTERNS, patternIds: MINOR6_PATTERN_IDS },
  },
  {
    slug: 'six-note-scale-7alt-m7b5',
    titleJa: '7alt(m7♭5)',
    titleEn: '7alt(m7♭5)',
    sortOrder: 3,
    referenceKey: 'C',
    chord: { chordName: 'G7alt(Fm7♭5)', patterns: ALT_M7B5_PATTERNS, patternIds: MINOR6_PATTERN_IDS },
  },
  {
    slug: 'six-note-scale-7alt-mm7-omit6',
  ...
  {
    slug: 'six-note-scale-maj7',
    titleJa: 'M7',
    titleEn: 'M7',
    sortOrder: 5,
    referenceKey: 'C',
    chord: { chordName: 'CM7', patterns: MAJOR_3_PATTERNS, patternIds: MAJOR3_PATTERN_IDS },
  },
  {
    slug: 'six-note-scale-mm7-omit6',
    titleJa: 'mM7(omit 6)',
    titleEn: 'mM7(omit 6)',
    sortOrder: 6,
    referenceKey: 'Eb',
  ...
  {
    slug: 'six-note-scale-mm7-omit4',
    titleJa: 'mM7(omit 4)',
    titleEn: 'mM7(omit 4)',
    sortOrder: 7,
    referenceKey: 'Eb',
    chord: { chordName: 'CmM7(omit 4)', patterns: MAJOR_3_PATTERNS, patternIds: MAJOR3_PATTERN_IDS },
  },
  {
    slug: 'six-note-scale-m7b5',
    titleJa: 'm7♭5',
    titleEn: 'm7♭5',
    sortOrder: 8,
    referenceKey: 'Eb',
    chord: { chordName: 'Am7♭5', patterns: M7B5_PATTERNS, patternIds: MINOR6_PATTERN_IDS },
  },
  {
    slug: 'six-note-scale-m7b5-mm7-omit6',
  ...
  {
    slug: 'six-note-scale-7-sharp11-mm7-omit6',
...
const PROGRESSION_SPECS: readonly {
  slug: string;
  titleJa: string;
  titleEn: string;
  sortOrder: number;
  ...
  {
    slug: 'six-note-scale-prog-ii-v7alt-mm7-omit6',
    ...
    template: {
      ...
      variants: [
        { patternIds: ['abc', 'abc'] },
        { patternIds: ['bca', 'bca'] },
        { patternIds: ['aprime_abc', 'aprime_abc'] },
      ],
    },
  },
  {
    slug: 'six-note-scale-prog-ii-v7alt-m7b5',
    ...
    template: {
      ...
      variants: [
        { patternIds: ['abc', 'abc'] },
        { patternIds: ['bca', 'abc_inversion'] },
        { patternIds: ['aprime_abc', 'aprime_abc'] },
      ],
    },
  },
  {
    slug: 'six-note-scale-prog-minor-ii-v-i',
    ...
    template: {
      ...
      variants: MINOR6_PATTERN_IDS.map((patternId) => ({
        patternIds: [patternId, patternId, patternId],
      })),
    },
  },
];
...
export const buildSixNoteScaleTrainingSpecs = (): readonly SixNoteScaleTrainingSpec[] => {
  const singles: SixNoteScaleTrainingSpec[] = SINGLE_SPECS.map((spec) => ({
    slug: spec.slug,
    ...
    progression: buildSingleChordProgression(
      spec.referenceKey,
      spec.chord.chordName,
      spec.chord.patterns,
      spec.chord.patternIds,
    ),
  }));

  const progressions: SixNoteScaleTrainingSpec[] = PROGRESSION_SPECS.map((spec) => ({
    slug: spec.slug,
    titleJa: spec.titleJa,
    titleEn: spec.titleEn,
    sortOrder: spec.sortOrder,
    unitSize: spec.template.chords.length,
    progression: buildProgressionTemplateEntries(spec.template),
  }));
