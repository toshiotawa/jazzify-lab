...
/** `lesson_songs` からネスト取得する `trainings` 行（一覧・詳細表示用） */
export interface TrainingStageRow {
  id: string;
  slug: string;
  title_ja: string;
  title_en?: string | null;
...
export interface LessonSong {
  ...
  /** false のとき任意課題（課題詳細に注意書きを表示。クエスト完了判定からは除外される） */
...
export interface Course {
  ...
  /** メインクエストの楽器向け（piano / all）。DB: main_quest_instrument */
  main_quest_instrument?: 'piano' | 'all' | null;
