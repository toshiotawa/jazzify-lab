...
export interface ChordOsmdSwingScope {
  partIndex: number;
  staff: 1 | 2;
}
...
export interface ChordOsmdNoteClusterContext {
  ...
  /** 0-indexed `<part>`。複数パート譜でスイング抑制をパート内に閉じる */
  partIndex: number;
  /** MusicXML `<staff>`。未指定は 1 */
  staff: 1 | 2;
}
...
const staffFromNoteElement = (noteEl: Element): 1 | 2 => (
  parseStaffNumber(getDirectChildText(noteEl, 'staff')) ?? 1
);
...
const selectDirectChildMeasures = (parent: Element): Element[] => {
  const measures: Element[] = [];
  for (const child of Array.from(parent.childNodes)) {
    if (isElementNode(child) && child.localName === 'measure') {
      measures.push(child);
    }
  }
  return measures;
...
const straightBeatGlobalKey = (measureNumber: number, beatWhole: number): string => (
  `${measureNumber}:${beatWhole}`
);

const straightBeatPartStaffKey = (
  partIndex: number,
  staff: 1 | 2,
  measureNumber: number,
  beatWhole: number,
): string => `${partIndex}:${staff}:${measureNumber}:${beatWhole}`;

/** 16分など 0/0.5 以外のオンセットを含む拍はスイングしない（`${measureNumber}:${beatWhole}` とパート/譜表キー） */
export const collectChordOsmdStraightBeatKeys = (
  musicXmlText: string,
): ReadonlySet<string> => {
  const straightKeys = new Set<string>();
  forEachChordOsmdNoteCluster(musicXmlText, ({ measureNumber, beatStartInMeasure, partIndex, staff }) => {
    ...
    if (!onStraightGrid) {
      straightKeys.add(straightBeatGlobalKey(measureNumber, beatWhole));
      straightKeys.add(straightBeatPartStaffKey(partIndex, staff, measureNumber, beatWhole));
    }
  });
  return straightKeys;
};
...
const isStraightBeatKey = (
  ...
  const beatWhole = Math.floor(rawBeatIndex + STRAIGHT_BEAT_FRACTION_EPS);
  if (swingScope) {
    return straightBeatKeys.has(
      straightBeatPartStaffKey(swingScope.partIndex, swingScope.staff, measureNumber, beatWhole),
    );
  }
  return straightBeatKeys.has(straightBeatGlobalKey(measureNumber, beatWhole));
};
...
const chordOsmdLyricTargetTimeSec = (
  ...
  const rawBeatIndex = Math.max(0, beatStartInMeasure - 1);
  const beatIndex = isSwing && !isStraightBeatKey(straightBeatKeys, measureNumber, rawBeatIndex, swingScope)
    ? applyChordOsmdSwingToBeatIndex(rawBeatIndex)
    : rawBeatIndex;
  return (measureIndex * bpmSafe + beatIndex) * beatDurationSec;
};

/** 小節・拍（1-indexed）から phrase タイムライン秒へ（even / swing 共通） */
export const chordOsmdTargetTimeSecFromMeasureBeat = chordOsmdLyricTargetTimeSec;
...
export const forEachChordOsmdNoteCluster = (
  ...
  const walkMeasures = (measures: readonly Element[], partIndex: number): void => {
      ...
      if (getDirectChild(noteEl, 'rest')) {
        if (noteHasLyricElements(noteEl)) {
          ...
          onCluster({
            ...
            partIndex,
            staff: staffFromNoteElement(noteEl),
          });
        }
  ...
  if (parts.length === 0) {
    walkMeasures(selectMusicXmlMeasures(doc), 0);
    return;
  }
  for (let partIndex = 0; partIndex < parts.length; partIndex += 1) {
    ...
    walkMeasures(selectDirectChildMeasures(part), partIndex);
  }
};
...
export const midiCountArray = (counts: ReadonlyMap<number, number>): ChordOsmdMidiCount[] => (
  Array.from(counts, ([midi, count]) => ({ midi, count }))
    .filter(item => item.count > 0)
    .sort((a, b) => a.midi - b.midi)
);

export const buildPlayableMeasureLabels = (
...
export const buildPlayableMeasures = (chords: readonly EarTrainingPhraseChord[]): Set<number> => {
...
export const buildDisabledMeasures = (chords: readonly EarTrainingPhraseChord[]): Set<number> => {
  const measures = new Set<number>();
  for (const chord of chords) {
    if (chord.input_disabled !== true) {
      continue;
 ...
 * fromScore でない／attacks が空のときのフォールバック。正本統一時は {@link buildCanonicalPhraseNotes} を優先。
 */
...
const buildChordOsmdRhythmTargetsFromScore = (
  ...
  const sortedAttacks = attacks
    ...
    .sort((a, b) => {
      const timeA = chordOsmdLyricTargetTimeSec(
        a.measureNumber,
        a.beatStartInMeasure,
        bpm,
        beatsPerMeasure,
        isSwing,
        straightBeatKeys,
      ...
      const timeB = chordOsmdLyricTargetTimeSec(
        b.measureNumber,
        b.beatStartInMeasure,
        bpm,
        beatsPerMeasure,
        isSwing,
        straightBeatKeys,
...
export const resolveChordOsmdParrySpanFinishMeasure = (
  anchorMeasureNumber: number,
  spanMeasures: number,
): number => anchorMeasureNumber + Math.max(1, spanMeasures);

export interface ChordOsmdParrySpanAnchor {
  id: string;
  measureNumber: number;
  targetTimeSec: number;
  orderIndex: number;
...
export const toChordOsmdParrySpanAnchor = (
  target: ChordOsmdRhythmTarget,
): ChordOsmdParrySpanAnchor => ({
  id: target.id,
...
export const findLastChordOsmdTargetOnBeatInMeasure = (
  ...
  for (const candidate of targets) {
    ...
    if (candidate.targetTimeSec + 1e-9 < beatStartSec) {
      continue;
    }
...
export const resolveChordOsmdParrySpanFinishTarget = (
  targets: readonly ChordOsmdRhythmTarget[],
  anchor: ChordOsmdParrySpanAnchor,
  spanMeasures: number,
  bpm: number,
...
export interface ChordOsmdParrySpanState {
  anchor: ChordOsmdParrySpanAnchor;
  finishTarget: ChordOsmdRhythmTarget | null;
  isFinish: boolean;
...
export const resolveChordOsmdParrySpanState = (
  ...
  const hadChain = activeAnchor !== null;
  if (activeAnchor === null) {
  ...
  const isFinish = finishTarget !== null && finishTarget.id === target.id;
