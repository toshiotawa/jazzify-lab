...
export type LoopTransposeDirection = 'down' | 'up';
...
export const scaleScoreSecToProcessedBufferSec = (
  scoreSec: number,
  speedPercent: number,
): number => {
  const ratio = Math.max(0.4, Math.min(1, speedPercent / 100));
  return scoreSec / ratio;
};
