...
export const getEarTrainingGameCopy = (isEnglish: boolean): EarTrainingGameCopy => (
  isEnglish
    ? {
        ...
        countIn: 'Count-in',
        transitionNextBar: rank => `${rank} — next phrase shortly before the bar line`,
    ...
    : {
        ...
        countIn: 'カウントイン',
        transitionNextBar: rank => `${rank}: 小節末直前で次へ`,
...
/** コードクイズ開始時の大バナー文言（必要正解数はステージ値をそのまま表示） */
export const formatEarTrainingChordQuizIntroLine = (
  isEnglish: boolean,
  requiredCorrectCount: number,
): string => {
  if (isEnglish) {
    return `Get ${requiredCorrectCount} correct to clear!`;
  }
  return `${requiredCorrectCount}問正解でクリア！`;
};
