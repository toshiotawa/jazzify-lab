...
const DEFAULT_SAMPLER_URLS: Record<string, string> = {
  A2: 'A2.mp3',
  'D#3': 'Ds3.mp3',
  A3: 'A3.mp3',
  C4: 'C4.mp3'
...
export const initializeAudioSystem = async (_opts?: { light?: boolean }): Promise<void> => {
  ...
  try {
    console.log('🎹 Initializing optimized audio system (lightweight sampler)...');

    await detectUserInteraction();

    if (typeof window === 'undefined') {
      throw new Error('Audio system is only available in browser environments');
    }
    ...
    if (!window.Tone) {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          retryCount++;
          console.warn(`⚠️ Tone.js dynamic import attempt ${retryCount} failed:`, toneError);
          if (retryCount >= maxRetries) {
            throw new Error(`Tone.jsの読み込みに失敗: ${toneError instanceof Error ? toneError.message : 'Unknown error'}`);
          }
    ...
    try {
      ...
      console.log('✅ Sampler audio samples loaded');
    } catch {
      console.warn('⚠️ Sampler samples failed to confirm loading, continuing optimistically');
    }

    audioSystemInitialized = true;
    console.log('✅ Optimized audio system initialized successfully (lightweight-only)');
  } catch (error) {
...
export const playNote = async (note: number, velocity: number = 127): Promise<void> => {
  try {
    ...
    if (!globalSampler) {
      throw new Error('Sampler not available');
    }
