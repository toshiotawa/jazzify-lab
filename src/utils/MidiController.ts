...
export const initializeAudioSystem = async (_opts?: { light?: boolean }): Promise<void> => {
  ...
  if (audioInitializationPromise) {
    await audioInitializationPromise;
    return;
  }
  ...
  audioInitializationPromise = (async () => {
    try {
      ...
      const optimizedContext = new ToneGlobal.Context({
        latencyHint: 'interactive',
        lookAhead: 0
      });
      ToneGlobal.setContext(optimizedContext);
      console.log('✅ Tone.js context optimized for low latency');

      if (!globalSampler) {
        globalSampler = new ToneGlobal.Sampler({
        ...
        if ((globalSampler as any).envelope) {
          ...
          (globalSampler as any).envelope.release = 1.2;
        }

        await ToneGlobal.loaded();
        console.log('✅ Lightweight sampler decoded and ready');
      }
...
export const playNote = async (note: number, velocity: number = 127): Promise<void> => {
  try {
    ...
    // 既に再生中のノートがある場合は一旦停止
    if (activeNotes.has(noteName) && globalSampler) {
      try {
