...
export const initializeAudioSystem = async (opts?: { light?: boolean }): Promise<void> => {
  ...
  try {
    ...
    if (!lightMode) {
      console.info('🎹 High quality piano mode has been retired; continuing with lightweight sampler only.');
    }
    ...
    console.log('✅ Lightweight sampler audio samples loaded');
...
export const playNote = async (note: number, velocity: number = 127): Promise<void> => {
  try {
    ...
    if (!globalSampler) {
      console.warn('⚠️ Lightweight sampler is not ready');
      return;
    }
...
export const stopNote = (note: number): void => {
  try {
    ...
    if (typeof globalSampler.triggerRelease !== 'function') {
      return;
    }
