...
export const initializeAudioSystem = async (opts?: { light?: boolean }): Promise<void> => {
  ...
  if (opts?.light === false) {
    console.info('🎹 High-quality piano mode is deprecated. Using lightweight sampler instead.');
  }
  ...
  try {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          if (retryCount >= maxRetries) {
            ...
            throw new Error(
              `音声/MIDIシステム初期化に失敗 (ユーザーインタラクション後に再試行): ${
                toneError instanceof Error ? toneError.message : 'Unknown error'
              }`
            );
          }
    ...
    if (globalSampler && (globalSampler as any).envelope) {
      ...
      (globalSampler as any).envelope.release = 0.4;
    }

    void (window.Tone as any).loaded().then(() => {
      console.log('✅ Lightweight sampler audio buffers ready');
    }).catch(() => {});
