...
export const initializeAudioSystem = async (_opts?: { light?: boolean }): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Sampler audio samples loaded (lightweight set)');
    } catch (loadError) {
      console.warn('⚠️ Sampler preload failed (audio will lazy-load):', loadError);
    }

    audioSystemInitialized = true;
    console.log('✅ Optimized audio system initialized successfully (light sampler only)');
