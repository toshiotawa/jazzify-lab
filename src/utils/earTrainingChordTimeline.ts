...
export const getEarTrainingChordDisplayAtTime = (
  ...
  for (let j = 0; j < n; j += 1) {
    const start = sorted[j].start_time_sec;
    if (j === 0) {
      adjStart[j] = start;
    } else {
  ...
  for (let j = 0; j < n; j += 1) {
    const endNominal =
      sorted[j].end_time_sec ?? (j + 1 < n ? sorted[j + 1].start_time_sec : Number.POSITIVE_INFINITY);
    if (completedChordIds.has(sorted[j].id) && j + 1 < n) {
      adjEnd[j] = sorted[j + 1].start_time_sec - half;
    } else {
