...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  const taikoNotesLength = gameState.taikoNotes.length;
  const lastTaikoNoteTime = taikoNotesLength > 0 ? gameState.taikoNotes[taikoNotesLength - 1].hitTime : 0;
  const taikoNotesRef = useRef<TaikoNote[]>(gameState.taikoNotes);
  useEffect(() => {
  ...
  const taikoLoopParamsRef = useRef<TaikoLoopParams | null>(null);
  useEffect(() => {
    const stageData = gameState.currentStage;
    if (!stageData || taikoNotesLength === 0) {
      taikoLoopParamsRef.current = null;
      return;
    }

    const bpm = Number(stageData.bpm) > 0 ? Number(stageData.bpm) : 120;
    const timeSignature = Number(stageData.timeSignature) > 0 ? Number(stageData.timeSignature) : 4;
    const secPerBeat = 60 / bpm;
    const secPerMeasure = secPerBeat * timeSignature;
    const measureCount = stageData.measureCount && stageData.measureCount > 0
      ? stageData.measureCount
      : Math.max(1, Math.ceil(taikoNotesLength / Math.max(1, Math.round(timeSignature))));
    const stageLoopDuration = Math.max(secPerMeasure, measureCount * secPerMeasure);
    const loopDuration = Math.max(stageLoopDuration, lastTaikoNoteTime + secPerBeat);
    const chordData = Array.isArray((stageData as any).chordProgressionData)
      ? ((stageData as any).chordProgressionData as Array<any>)
      : [];
    const overlayMarkers = chordData
      .filter((entry) => entry && typeof entry.text === 'string' && entry.text.trim() !== '')
      .map((entry) => ({
        time: (entry.bar - 1) * secPerMeasure + ((entry.beats ?? 1) - 1) * secPerBeat,
        text: entry.text.trim()
      }))
      .sort((a, b) => a.time - b.time);

    taikoLoopParamsRef.current = {
      secPerBeat,
      secPerMeasure,
      loopDuration,
  ...
  }, [gameState.currentStage, taikoNotesLength, lastTaikoNoteTime]);
  ...
  useEffect(() => {
    ...
    // 太鼓の達人モードのノーツ表示更新（unifiedFrameController統合版）
    useEffect(() => {
      if (!fantasyPixiInstance || !gameState.isTaikoMode) return;

      let animationId: number | null = null;
      let cancelled = false;
      const controller = unifiedFrameController;
      ...
      const updateTaikoNotes = (timestamp: number) => {
        if (cancelled) return;
        if (controller.shouldSkipFrame(timestamp, 'render')) {
          animationId = requestAnimationFrame(updateTaikoNotes);
          return;
        }
        if (!controller.shouldUpdateNotes(timestamp)) {
          animationId = requestAnimationFrame(updateTaikoNotes);
          return;
        }
        controller.markNoteUpdate(timestamp);

        const params = taikoLoopParamsRef.current;
        const taikoNotes = taikoNotesRef.current;
        if (!params || !taikoNotes || taikoNotes.length === 0) {
          animationId = requestAnimationFrame(updateTaikoNotes);
          return;
        }
        ...
        const { loopDuration, previewWindow, overlayMarkers } = params;
        ...
        const pushNote = (payload: { id: string; chord: string; x: number }): boolean => {
          if (notesToDisplay.length >= PERFORMANCE_CONFIG.MAX_VISIBLE_NOTES) {
            return false;
          }
          notesToDisplay.push(payload);
          return true;
        };

        if (currentTime < 0) {
          for (let i = 0; i < taikoNotes.length && notesToDisplay.length < maxPreCountNotes; i++) {
            ...
            if (timeUntilHit >= -0.5) {
              ...
              if (!pushNote({ id: note.id, chord: note.chord.displayName, x })) {
                break;
              }
            }
          }
        ...
        const currentNoteIndexSnapshot = currentNoteIndexRef.current;
        const totalNotes = taikoNotes.length;

        for (let i = currentNoteIndexSnapshot; i < totalNotes; i++) {
          const note = taikoNotes[i];
          if (note.isHit) {
            continue;
          }
          const timeUntilHit = note.hitTime - normalizedTime;
          if (timeUntilHit < lowerBound) {
            continue;
          }
        ...
        const lastCompletedIndex = totalNotes > 0
          ? (currentNoteIndexSnapshot - 1 + totalNotes) % totalNotes
          : -1;
        const timeToLoop = loopDuration - normalizedTime;
        if (timeToLoop < previewWindow && totalNotes > 0) {
          for (let i = 0; i < totalNotes; i++) {
            if (notesToDisplay.length >= PERFORMANCE_CONFIG.MAX_VISIBLE_NOTES) {
              break;
            }
            if (i === lastCompletedIndex || i === currentNoteIndexSnapshot) {
              continue;
            }
            const note = taikoNotes[i];
            ...
            const x = judgeLinePos.x + timeUntilHit * noteSpeed;
            if (!pushNote({ id: `${note.id}_loop`, chord: note.chord.displayName, x })) {
              break;
            }
          }
        }
        ...
        fantasyPixiInstance.updateTaikoNotes(notesToDisplay);
        fantasyPixiInstance.updateOverlayText(resolveOverlayLabel(normalizedTime, overlayMarkers));
        animationId = requestAnimationFrame(updateTaikoNotes);
      };
