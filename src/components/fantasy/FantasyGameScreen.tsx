...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  useEffect(() => {
    taikoLoopStateRef.current.notes = gameState.taikoNotes;
  }, [gameState.taikoNotes]);

  useEffect(() => {
    taikoLoopStateRef.current.currentIndex = gameState.currentNoteIndex;
  }, [gameState.currentNoteIndex]);

  useEffect(() => {
    taikoLoopStateRef.current.isTaikoMode = gameState.isTaikoMode;
  }, [gameState.isTaikoMode]);

  useEffect(() => {
    const stageMeta = gameState.currentStage ?? stage;
    taikoLoopStateRef.current.stageMeta = stageMeta;
    if (stageMeta && Array.isArray((stageMeta as any).chordProgressionData)) {
      const bpm = stageMeta.bpm || 120;
      const secPerBeat = 60 / bpm;
      const secPerMeasure = secPerBeat * (stageMeta.timeSignature || 4);
      taikoOverlayMarkersRef.current = ((stageMeta as any).chordProgressionData as Array<any>)
  ...
  useEffect(() => {
    ...
    const updateTaikoNotes = (timestamp: number) => {
      ...
      const { notes, currentIndex, isTaikoMode, stageMeta } = taikoLoopStateRef.current;
      if (!isTaikoMode || notes.length === 0) {
        fantasyPixiInstance.updateTaikoNotes([]);
        fantasyPixiInstance.updateOverlayText(null);
        return;
      }

      const timingStage = stageMeta ?? stage;
      const bpm = timingStage?.bpm || 120;
      const timeSignature = timingStage?.timeSignature || 4;
      const measureCount = timingStage?.measureCount ?? 8;
      const secPerBeat = 60 / bpm;
      ...
      const lastCompletedIndex = notes.length > 0 ? (currentIndex - 1 + notes.length) % notes.length : -1;
