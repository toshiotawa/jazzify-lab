...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const taikoCurrentIndexRef = useRef(0);
    const taikoLoopDurationRef = useRef(0);
    const taikoOverlayMarkersRef = useRef<Array<{ time: number; text: string }>>([]);
    const taikoRenderBufferRef = useRef<Array<{ id: string; chord: string; x: number }>>([]);
    const taikoDisplayedBaseIdRef = useRef<Set<string>>(new Set());
    const taikoStageRef = useRef<FantasyStage | null>(null);
  ...
  const {
    ...
    useEffect(() => {
      taikoCurrentIndexRef.current = gameState.currentNoteIndex;
    }, [gameState.currentNoteIndex]);

    useEffect(() => {
      taikoStageRef.current = gameState.currentStage ?? null;
      if (!gameState.currentStage) {
        taikoLoopDurationRef.current = 0;
        taikoOverlayMarkersRef.current = [];
        return;
      }
      const stageInfo = gameState.currentStage;
      const secPerBeat = 60 / (stageInfo.bpm || 120);
      const secPerMeasure = secPerBeat * (stageInfo.timeSignature || 4);
      taikoLoopDurationRef.current = (stageInfo.measureCount || 8) * secPerMeasure;
      taikoOverlayMarkersRef.current = Array.isArray((stageInfo as any).chordProgressionData)
        ? ((stageInfo as any).chordProgressionData as Array<any>)
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const updateTaikoFrame = (timestamp: number) => {
        if (!fantasyPixiInstance) {
          return;
        }
        
        if (unifiedFrameController.shouldSkipFrame(timestamp, 'render')) {
          animationId = requestAnimationFrame(updateTaikoFrame);
          return;
        }
        ...
        const stageInfo = taikoStageRef.current;
        const loopDuration = taikoLoopDurationRef.current;
        ...
        const buffer = taikoRenderBufferRef.current;
        buffer.length = 0;
        const displayedBaseIds = taikoDisplayedBaseIdRef.current;
        displayedBaseIds.clear();
        
        const pushNote = (id: string, chordName: string, x: number) => {
          buffer.push({ id, chord: chordName, x });
        };
        
        const currentTime = bgmManager.getCurrentMusicTime();
        
        if (!isTaikoModeRef.current || !stageInfo || taikoNotes.length === 0) {
        ...
        if (currentTime < 0) {
          ...
          for (let i = 0; i < taikoNotes.length; i++) {
            ...
            if (timeUntilHit >= -0.5) {
              ...
              pushNote(note.id, note.chord.displayName, x);
              if (buffer.length >= maxPreCountNotes) break;
            }
          }
        ...
        const normalizedTime = loopDuration > 0
          ? ((currentTime % loopDuration) + loopDuration) % loopDuration
          : currentTime;
        const currentNoteIndex = taikoCurrentIndexRef.current;
        const lowerBound = -0.35;
        
        for (let index = 0; index < taikoNotes.length; index++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
          ...
          const baseId = id.endsWith('_loop') ? id.slice(0, -5) : id;
          displayedBaseIds.add(baseId);
        }
        
        if (loopDuration > 0 && taikoNotes.length > 0) {
          const lastCompletedIndex = taikoNotes.length > 0
            ? (currentNoteIndex - 1 + taikoNotes.length) % taikoNotes.length
            : -1;
          const timeToLoop = loopDuration - normalizedTime;
          
          if (timeToLoop < previewWindow) {
            for (let i = 0; i < taikoNotes.length; i++) {
              const note = taikoNotes[i];
              ...
              const x = judgeLinePos.x + timeUntilHit * noteSpeed;
              pushNote(`${note.id}_loop`, note.chord.displayName, x);
            }
          }
        }
        ...
        if (overlayMarkers.length > 0 && loopDuration > 0) {
