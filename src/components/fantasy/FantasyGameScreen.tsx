...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  // 太鼓の達人モードのノーツ表示更新（unifiedFrameController準拠）
  useEffect(() => {
    if (
      !fantasyPixiInstance ||
      !gameState.isTaikoMode ||
    ...
    const taikoNotes = gameState.taikoNotes;
    const noteCount = taikoNotes.length;
    const controller =
      (typeof window !== 'undefined' && (window as any).unifiedFrameController)
        ? (window as any).unifiedFrameController
    ...
    const overlayMarkers: Array<{ time: number; text: string }> = Array.isArray(
      (stage as any).chordProgressionData
    )
    ...
    const notesBuffer: Array<{ id: string; chord: string; x: number }> = [];
    const displayedIds = new Set<string>();
    let animationId: number | null = null;

    const updateTaikoNotes = (timestamp: number) => {
      if (controller.shouldSkipFrame(timestamp, 'logic') || !controller.shouldUpdateNotes(timestamp)) {
      ...
      if (currentTime < 0) {
        for (let i = 0; i < noteCount && notesBuffer.length < maxPreCountNotes; i++) {
          const note = taikoNotes[i];
          const timeUntilHit = note.hitTime - currentTime;
          if (timeUntilHit > lookAheadTime || timeUntilHit < -0.5) {
            continue;
          }
          notesBuffer.push({
            id: note.id,
        ...
        fantasyPixiInstance.updateTaikoNotes(notesBuffer);
        if (overlayMarkers.length > 0) {
          fantasyPixiInstance.updateOverlayText(resolveOverlayLabel(0));
        } else {
      ...
      const currentNoteIndex = gameState.currentNoteIndex;

      for (let i = currentNoteIndex; i < noteCount; i++) {
        const note = taikoNotes[i];
        if (note.isHit) {
          continue;
        }
        ...
        notesBuffer.push({ id: note.id, chord: note.chord.displayName, x });
        displayedIds.add(note.id);
      }

      const lastCompletedIndex =
        noteCount > 0 ? (currentNoteIndex - 1 + noteCount) % noteCount : -1;
      const timeToLoop = loopDuration - normalizedTime;

      if (timeToLoop < previewWindow && noteCount > 0) {
        for (let i = 0; i < noteCount; i++) {
          if (i === lastCompletedIndex || i === currentNoteIndex) continue;
          const note = taikoNotes[i];
      ...
      fantasyPixiInstance.updateTaikoNotes(notesBuffer);
      if (overlayMarkers.length > 0) {
        fantasyPixiInstance.updateOverlayText(resolveOverlayLabel(normalizedTime));
      } else {
        fantasyPixiInstance.updateOverlayText(null);
      }
