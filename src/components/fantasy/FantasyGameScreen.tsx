...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  useEffect(() => {
    if (!isReady) {
      bgmManager.play(
        ...
        // ★ 互換: デモ開始時に音源アップグレードを試行（現在は軽量シンセ固定）
      (async () => {
        try {
  ...
  const taikoAnimationRef = useRef<number | null>(null);
  const taikoRuntimeRef = useRef<TaikoRuntimeState>({
    taikoNotes: [],
    currentNoteIndex: 0,
    isTaikoMode: false
  });
  const taikoTimingRef = useRef<TaikoTimingMeta>({
    loopDuration: 0,
    lookAheadTime: PERFORMANCE_CONFIG.LOOK_AHEAD_TIME,
    previewWindow: 0,
    secPerBeat: 0,
    secPerMeasure: 0,
    overlayMarkers: []
  ...
  useEffect(() => {
    taikoRuntimeRef.current = {
      taikoNotes: gameState.taikoNotes,
      currentNoteIndex: gameState.currentNoteIndex,
      isTaikoMode: gameState.isTaikoMode
    };
  }, [gameState.taikoNotes, gameState.currentNoteIndex, gameState.isTaikoMode]);

  useEffect(() => {
    const stage = gameState.currentStage;
    if (!stage) {
      taikoTimingRef.current = {
        loopDuration: 0,
        lookAheadTime: PERFORMANCE_CONFIG.LOOK_AHEAD_TIME,
        previewWindow: 0,
        secPerBeat: 0,
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      if (timestamp - lastUpdateTime >= updateInterval) {
        ...
        const frame = buildTaikoFrame(taikoRuntimeRef.current, taikoTimingRef.current, fantasyPixiInstance);
        if (taikoRuntimeRef.current.isTaikoMode && taikoRuntimeRef.current.taikoNotes.length > 0) {
          fantasyPixiInstance.updateTaikoNotes(frame.notes);
          fantasyPixiInstance.updateOverlayText(frame.overlay);
        } else {
          fantasyPixiInstance.updateTaikoNotes([]);
          fantasyPixiInstance.updateOverlayText(null);
        }
      }
      taikoAnimationRef.current = requestAnimationFrame(tick);
    };
    ...
    return () => {
      disposed = true;
      if (taikoAnimationRef.current) {
        cancelAnimationFrame(taikoAnimationRef.current);
        taikoAnimationRef.current = null;
      }
    };
  }, [fantasyPixiInstance]);
...
interface TaikoRuntimeState {
  taikoNotes: TaikoNote[];
  currentNoteIndex: number;
  isTaikoMode: boolean;
}

interface TaikoTimingMeta {
  loopDuration: number;
...
const LEFT_BOUND_SECONDS = -0.35;
const COUNT_IN_LOWER_BOUND = -0.5;
const MAX_COUNTDOWN_NOTES = 6;

const buildTaikoFrame = (
  runtime: TaikoRuntimeState,
  timing: TaikoTimingMeta,
  pixiInstance: FantasyPIXIInstance
): { notes: TaikoDisplayNote[]; overlay: string | null } => {
  if (!runtime.isTaikoMode || runtime.taikoNotes.length === 0 || timing.loopDuration <= 0) {
    return { notes: [], overlay: null };
  }

  const judgeLinePos = pixiInstance.getJudgeLinePosition();
  const currentTime = bgmManager.getCurrentMusicTime();
  const lookAheadTime = timing.lookAheadTime || PERFORMANCE_CONFIG.LOOK_AHEAD_TIME;

  if (currentTime < 0) {
    const countdownNotes: TaikoDisplayNote[] = [];
    for (let i = 0; i < runtime.taikoNotes.length; i++) {
      const note = runtime.taikoNotes[i];
      const timeUntilHit = note.hitTime - currentTime;
      if (timeUntilHit > lookAheadTime) break;
      if (timeUntilHit >= COUNT_IN_LOWER_BOUND) {
        countdownNotes.push({
          id: note.id,
          chord: note.chord.displayName,
          x: judgeLinePos.x + timeUntilHit * NOTE_SPEED
        });
        if (countdownNotes.length >= MAX_COUNTDOWN_NOTES) {
          break;
        }
      }
    }
    return {
      notes: capNotes(countdownNotes),
      overlay: timing.overlayMarkers[0]?.text ?? null
    };
  }

  const normalizedTime = ((currentTime % timing.loopDuration) + timing.loopDuration) % timing.loopDuration;
  const visibleNotes: TaikoDisplayNote[] = [];

  runtime.taikoNotes.forEach((note, index) => {
    if (note.isHit) return;
    if (index < runtime.currentNoteIndex) return;
    const timeUntilHit = note.hitTime - normalizedTime;
    if (timeUntilHit >= LEFT_BOUND_SECONDS && timeUntilHit <= lookAheadTime) {
      visibleNotes.push({
        id: note.id,
        chord: note.chord.displayName,
        x: judgeLinePos.x + timeUntilHit * NOTE_SPEED
  ...
  const displayedIds = new Set(visibleNotes.map((n) => n.id));
  const lastCompletedIndex = runtime.taikoNotes.length
    ? (runtime.currentNoteIndex - 1 + runtime.taikoNotes.length) % runtime.taikoNotes.length
    : -1;
  const timeToLoop = timing.loopDuration - normalizedTime;

  if (timeToLoop < timing.previewWindow && runtime.taikoNotes.length > 0) {
    for (let i = 0; i < runtime.taikoNotes.length; i++) {
      if (i === lastCompletedIndex) continue;
      if (i === runtime.currentNoteIndex) continue;
      const note = runtime.taikoNotes[i];
      if (displayedIds.has(note.id)) continue;
      const virtualHitTime = note.hitTime + timing.loopDuration;
      const timeUntilHit = virtualHitTime - normalizedTime;
      if (timeUntilHit <= 0 || timeUntilHit > timing.previewWindow) continue;
      visibleNotes.push({
        id: `${note.id}_loop`,
        chord: note.chord.displayName,
        x: judgeLinePos.x + timeUntilHit * NOTE_SPEED
      });
      if (visibleNotes.length >= PERFORMANCE_CONFIG.MAX_VISIBLE_NOTES) {
        break;
      }
    }
  }
  ...
  return {
    notes: capNotes(visibleNotes),
    overlay: selectOverlayLabel(normalizedTime, timing.overlayMarkers)
  };
};

const capNotes = (notes: TaikoDisplayNote[]): TaikoDisplayNote[] => {
  if (notes.length <= PERFORMANCE_CONFIG.MAX_VISIBLE_NOTES) {
    return notes;
  }
  return notes.slice(0, PERFORMANCE_CONFIG.MAX_VISIBLE_NOTES);
};

const selectOverlayLabel = (
