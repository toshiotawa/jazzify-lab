...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const maxVisible = PERFORMANCE_CONFIG.MAX_VISIBLE_NOTES ?? 20;
      ...
      const updateTaikoNotes = (timestamp: number) => {
        ...
        const timeToLoopEnd = loopDuration - normalizedTime;
        const nearLoopBoundary = timeToLoopEnd < previewWindow;
        
        const notesDetailed: Array<{id: string; chord: string; x: number; timeUntilHit: number}> = [];
        ...
        gameState.taikoNotes.forEach((note, index) => {
          ...
          if (loopOffset === 0 && index < gameState.currentNoteIndex) {
            return;
          }
          
          const windowLimit = loopOffset === 0 ? lookAheadTime : previewWindow;
          if (timeUntilHit > windowLimit || timeUntilHit < lowerBound) {
            return;
          }
          
          const x = judgeLinePos.x + timeUntilHit * noteSpeed;
          notesDetailed.push({
            id: loopOffset === 0 ? note.id : `${note.id}_loop`,
        ...
        notesDetailed.sort((a, b) => a.timeUntilHit - b.timeUntilHit);
        if (notesDetailed.length > maxVisible) {
          notesDetailed.length = maxVisible;
        }
        
        fantasyPixiInstance.updateTaikoNotes(
          notesDetailed.map(({ id, chord, x }) => ({ id, chord, x }))
        );
