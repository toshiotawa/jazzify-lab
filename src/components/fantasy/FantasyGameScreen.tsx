...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  const taikoIndexRef = useRef(0);
  const taikoLoopConfigRef = useRef<{
    secPerBeat: number;
  ...
  useEffect(() => {
    if (!gameState.currentStage) {
      taikoLoopConfigRef.current = null;
      return;
    }
    const stageConfig = gameState.currentStage;
    const bpm = stageConfig.bpm || 120;
    const timeSignature = stageConfig.timeSignature || 4;
    const measureCount = stageConfig.measureCount || 8;
    const secPerBeat = 60 / bpm;
    const secPerMeasure = secPerBeat * timeSignature;
    const loopDuration = measureCount * secPerMeasure;
    const overlayMarkers: Array<{ time: number; text: string }> = Array.isArray((stageConfig as any).chordProgressionData)
      ? ((stageConfig as any).chordProgressionData as Array<any>)
  ...
  useEffect(() => {
    ...
    const updateTaikoNotes = (timestamp: number) => {
      ...
      const config = taikoLoopConfigRef.current;
      const notes = taikoNotesRef.current;

      if (!config || !notes || notes.length === 0) {
      ...
      if (currentTime < 0) {
        ...
        for (let i = 0; i < notes.length; i++) {
          ...
          if (timeUntilHit > config.lookAheadTime) break;
          if (timeUntilHit >= -0.5) {
            const x = judgeLinePos.x + timeUntilHit * config.noteSpeed;
      ...
      const normalizedTime = ((currentTime % config.loopDuration) + config.loopDuration) % config.loopDuration;
      const notesToDisplay: Array<{id: string, chord: string, x: number}> = [];

      notes.forEach((note, index) => {
        if (note.isHit) return;
        if (index < taikoIndexRef.current) return;

        const timeUntilHit = note.hitTime - normalizedTime;
        const lowerBound = -0.35;

        if (timeUntilHit >= lowerBound && timeUntilHit <= config.lookAheadTime) {
          const x = judgeLinePos.x + timeUntilHit * config.noteSpeed;
          notesToDisplay.push({
            id: note.id,
      ...
      const lastCompletedIndex = notes.length > 0
        ? (taikoIndexRef.current - 1 + notes.length) % notes.length
        : -1;

      const timeToLoop = config.loopDuration - normalizedTime;
      if (timeToLoop < config.previewWindow && notes.length > 0) {
        for (let i = 0; i < notes.length; i++) {
          const note = notes[i];
          if (i === lastCompletedIndex) continue;
          if (i === taikoIndexRef.current) continue;
          if (displayedBaseIds.has(note.id)) continue;

          const virtualHitTime = note.hitTime + config.loopDuration;
          const timeUntilHit = virtualHitTime - normalizedTime;
          if (timeUntilHit <= 0) continue;
          if (timeUntilHit > config.previewWindow) break;
      ...
      if (config.overlayMarkers.length > 0) {
        const t = normalizedTime;
        let label = config.overlayMarkers[config.overlayMarkers.length - 1].text;
        for (let i = 0; i < config.overlayMarkers.length; i++) {
          const cur = config.overlayMarkers[i];
          const next = config.overlayMarkers[i + 1];
          if (t >= cur.time && (!next || t < next.time)) {
            label = cur.text;
            break;
          }
          if (t < config.overlayMarkers[0].time) {
            label = config.overlayMarkers[config.overlayMarkers.length - 1].text;
          }
        }
