...
const FantasyGameScreen: React.FC<FantasyGameScreenProps> = ({
  ...
  const taikoIndexRef = useRef(gameState.currentNoteIndex);
  const taikoStageRef = useRef<FantasyStage | null>(gameState.currentStage ?? null);
  ...
  useEffect(() => {
    taikoIndexRef.current = gameState.currentNoteIndex;
  }, [gameState.currentNoteIndex]);

  useEffect(() => {
    taikoStageRef.current = gameState.currentStage ?? null;
    const stageSpec = gameState.currentStage;
    if (!stageSpec) {
    ...
    const bpm = stageSpec.bpm || 120;
    const timeSig = stageSpec.timeSignature || 4;
    const secPerBeat = 60 / bpm;
    const secPerMeasure = secPerBeat * timeSig;
    const loopDuration = (stageSpec.measureCount || 8) * secPerMeasure;
    ...
    taikoOverlayMarkersRef.current = Array.isArray((stageSpec as any).chordProgressionData)
      ? ((stageSpec as any).chordProgressionData as Array<any>)
  ...
  useEffect(() => {
    ...
    const updateTaikoNotes = (timestamp: number) => {
      ...
      if (controller && controller.shouldSkipFrame(timestamp, 'render')) {
      ...
      const timing = taikoTimingRef.current;
      if (!taikoNotes.length || timing.loopDuration <= 0) {
        fantasyPixiInstance.updateTaikoNotes([]);
        animationId = requestAnimationFrame(updateTaikoNotes);
        return;
      }
      ...
      const currentIndex = taikoIndexRef.current;
      ...
      for (let index = 0; index < taikoNotes.length; index++) {
        ...
        if (timeUntilHit < lowerBound || timeUntilHit > lookAheadTime) continue;
      ...
      const lastCompletedIndex = taikoNotes.length > 0
        ? (currentIndex - 1 + taikoNotes.length) % taikoNotes.length
        : -1;
      const timeToLoop = timing.loopDuration - normalizedTime;

      if (timeToLoop < timing.previewWindow && taikoNotes.length > 0) {
        for (let i = 0; i < taikoNotes.length; i++) {
          const note = taikoNotes[i];
          ...
          if (timeUntilHit > timing.previewWindow) break;
