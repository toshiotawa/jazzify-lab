...
export function generateBasicProgressionNotes(
  ...
  for (let measure = 1; measure <= measureCount; measure++) {
    ...
    if (chord) {
      // カウントインを考慮したhitTime計算
      const hitTime = (measure - 1) * secPerMeasure + countInDuration;
...
export function parseChordProgressionData(
  ...
  progressionData.forEach((item, index) => {
    ...
    if (chord) {
      ...
      // カウントインを考慮
      const hitTime = (item.bar - 1) * secPerMeasure + (item.beats - 1) * secPerBeat + countInDuration;
