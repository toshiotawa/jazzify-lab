...
interface FantasySheetMusicDisplayProps {
  ...
  simpleMode?: boolean;
  /** 次セクションのmusicXml（背景プリレンダリング用） */
  nextMusicXml?: string;
  /** 次セクションのBPM（プリレンダリング用） */
  nextBpm?: number;
  /** 次セクションの拍子（プリレンダリング用） */
...
const FantasySheetMusicDisplay: React.FC<FantasySheetMusicDisplayProps> = ({
  ...
  // 次セクションの楽譜を背景でプリレンダリング（セクション切り替え時の即時表示用）
  useEffect(() => {
    if (!nextMusicXml || !preRenderContainerRef.current) return;
    ...
    if (sheetRenderCache.has(cacheKey)) return;
