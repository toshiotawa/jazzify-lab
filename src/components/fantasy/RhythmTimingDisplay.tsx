...
export const RhythmTimingDisplay: React.FC<RhythmTimingDisplayProps> = ({
  ...
  const showJudgment = (type: 'success' | 'miss') => {
  ...
  return (
    <div className={`rhythm-timing-display ${className}`}>
      {/* Timing Track */}
...
export const useJudgmentFeedback = () => {
  const [displayRef, setDisplayRef] = useState<{ showJudgment: (type: 'success' | 'miss') => void } | null>(null)

  const triggerJudgment = (type: 'success' | 'miss') => {
    displayRef?.showJudgment(type)
  }

  return { setDisplayRef, triggerJudgment }
}