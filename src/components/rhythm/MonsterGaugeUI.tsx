...
interface Props {
  monsters: (MonsterState & { rhythmQuestion?: RhythmQuestion })[];
}

const barCls =
  'h-2 bg-gradient-to-r from-yellow-500 to-yellow-400 rounded-full transition-all duration-100';

export const MonsterGaugeUI: React.FC<Props> = ({ monsters }) => {
  ...
  return (
    <div className="flex justify-center gap-3 mt-2 px-4">
      {monsters.map((m) => {
        if (!m.rhythmQuestion) return null;
        
        const gauge = useGaugePercent(m.rhythmQuestion.absSec);
        const isActive = gauge > 0 && gauge < 80;
        
        return (
          <div key={m.id} className="flex-1 max-w-32">
            <div className="text-center text-xs text-gray-300 mb-1">
              {m.rhythmQuestion.chord}
            </div>
