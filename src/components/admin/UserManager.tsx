...
import { fetchUserLessonProgress, updateLessonProgress, unlockLesson, unlockBlock, resetBlockUnlock, LessonProgress, LESSON_PROGRESS_CACHE_KEY } from '@/platform/supabaseLessonProgress';
    ...
    setUserLessonProgress((prev) =>
      prev.map((p) =>
        blockLessonsCache[blockNumber]?.includes(p.lesson_id)
          ? { ...p, is_unlocked: shouldUnlock ? true : undefined }
          : p
      )
    );

    try {
      const api = shouldUnlock ? unlockBlock : resetBlockUnlock;
                      ...
                      <div className="flex items-center space-x-2">
                        {userCourseUnlockStatus[selectedCourse.id] === true ? (
                          ...
                          <div className="flex items-center space-x-1 text-orange-400">
                            ...
                            <span className="text-sm font-medium">旧ロック状態（要リセット）</span>
                          </div>
                        ) : (
