...
import BackgroundWall from '@/components/survival/descent/parts/BackgroundWall';
...
const DefenseDescentMap: React.FC<DefenseDescentMapProps> = ({
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const [loadedBlocks, loadedNodes, loadedClears] = await Promise.all([
  ...
  useEffect(() => {
    void reload();
    void preloadDescentImages().then(() => setAssetsReady(true));
  }, [reload]);
  ...
  return (
    <div className="relative flex h-[calc(100dvh-56px)] flex-col bg-[#09070f]">
      <style>{`
      ...
      {isMobileLayout && isMobileDetailOpen && selectedNode && (
        <div className="fixed inset-0 z-40 flex items-end md:hidden" role="dialog" aria-modal="true">
          ...
          />
          <div className="relative z-10 flex max-h-[85vh] w-full flex-col rounded-t-2xl border-t border-amber-500/30 bg-gradient-to-b from-[#140c1f]/95 to-[#060410]/95 shadow-[0_-10px_40px_rgba(0,0,0,0.6)]">
...
export default DefenseDescentMap;
