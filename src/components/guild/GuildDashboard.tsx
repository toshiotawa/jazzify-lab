...
const GuildDashboard: React.FC = () => {
                        ...
                        <div className="w-full h-full flex flex-col bg-gradient-game text-white">
                                ...
                                <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                                        <div className="max-w-4xl mx-auto space-y-4 text-center">
                                                ...
                                                {lastGuildInfo && (
                                                        <div className="mt-6 max-w-xl mx-auto text左 bg-slate-800 border border-slate-700 rounded p-4">
	...
	const mvpXp = memberMonthly.find(x => x.user_id === mvpUserId)?.monthly_xp || 0;
        ...
        return (
                        <div className="w-full h-full flex flex-col bg-gradient-game text白 text-white">
                                        <GameHeader />
                                        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
													...
													<li key={m.user_id} className="flex items-center gap-3">
														...
														<div className="flex-1 min-w-0">
															<div className="flex items-center gap-2">
																<button onClick={()=>{ window.location.hash = `#diary-user?id=${m.user_id}`; }} className="font-semibold text-lg truncate text-left hover:text-blue-400">{m.nickname}</button>
																{myGuild.guild_type === 'challenge' && (
																	<span className="text-xs px-2 py-0.5 rounded-full bg-green-700/40 border border-green-600 text-green-300 whitespace-nowrap">
																		{(() => {
																			const s = streaks[m.user_id];
																			const days = s?.daysCurrentStreak || 0;
																			const lvl = days > 0 ? Math.min(6, Math.ceil(days / 5)) : 0;
																			return `Streak Lv.${lvl}`;
																		})()}
																	</span>
																)}
																{m.selected_title && (
