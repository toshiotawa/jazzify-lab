...
type MonthData = {
	...
	mvp: { user_id: string; nickname: string; avatar_url?: string; contributed_xp: number } | null;
};
									...
									return (
										<div key={m} className="bg-slate-800 border border-slate-700 rounded p-4">
											<div className="flex items-center justify-between mb-3">
												...
												{d?.mvp ? (
													<div className="inline-flex items-center gap-2 bg-slate-900 rounded px-2 py-1 border border-slate-700">
														<span className="text-xs text-gray-400">MVP</span>
														<button onClick={()=>{ window.location.hash = `#diary-user?id=${d.mvp!.user_id}`; }} aria-label="ユーザー詳細へ">
															<img src={d.mvp.avatar_url || DEFAULT_AVATAR_URL} className="w-6 h-6 rounded-full" />
														</button>
														<button className="text-sm hover:text-blue-400" onClick={()=>{ window.location.hash = `#diary-user?id=${d.mvp!.user_id}`; }}>{d.mvp.nickname}</button>
														<span className="text-xs text-yellow-400">+{d.mvp.contributed_xp.toLocaleString()} XP</span>
													</div>
												) : (
															...
															<ul className="space-y-2">
																{d.contributors.map((u) => (
																	<li key={`${m}-${u.user_id}`} className="flex items-center gap-2 text-sm">
																		<button onClick={()=>{ window.location.hash = `#diary-user?id=${u.user_id}`; }} aria-label="ユーザー詳細へ">
																			<img src={u.avatar_url || DEFAULT_AVATAR_URL} className="w-6 h-6 rounded-full" />
																		</button>
																		<button className="truncate max-w-[200px] text-left hover:text-blue-400" onClick={()=>{ window.location.hash = `#diary-user?id=${u.user_id}`; }}>
																			{u.nickname}
																		</button>
																		{/* XPは表示しない指定 */}
																	</li>
