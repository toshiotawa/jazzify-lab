...
import { Guild, getGuildById, getGuildMembers, fetchGuildMemberMonthlyXp, fetchGuildRankForMonth, fetchGuildMonthlyXpSingle, requestJoin, getMyGuild, getMyPendingJoinRequestForGuild, cancelJoinRequest, fetchGuildMemberCount } from '@/platform/supabaseGuilds';
...
const GuildPage: React.FC = () => {
  ...
  const [myPendingReqId, setMyPendingReqId] = useState<string | null>(null);
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          {loading ? (
            ...
            <>
              <div className="bg-slate-800 border border-slate-700 rounded p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {!isMember && (
                      myPendingReqId ? (
                        <button className="btn btn-sm btn-outline" disabled={busy} onClick={async()=>{ try{ setBusy(true); await cancelJoinRequest(myPendingReqId); setMyPendingReqId(null); alert('申請をキャンセルしました'); } catch(e:any){ alert(e?.message||'キャンセルに失敗しました'); } finally{ setBusy(false); } }}>申請をキャンセル</button>
                      ) : (
                        guild.members_count < 5 ? (
                          <button className="btn btn-sm btn-primary" disabled={busy} onClick={async()=>{ try{ setBusy(true); const id = await requestJoin(guild.id); setMyPendingReqId(id); alert('参加リクエストを送信しました'); } catch(e:any){ alert(e?.message||'リクエスト送信に失敗しました'); } finally{ setBusy(false); } }}>参加リクエスト</button>
                        ) : (
                          <span className="text-sm text-red-400">満員のため申請できません</span>
                        )
                      )
                    )}
                  </div>
              ...
              <div className="bg-slate-800 border border-slate-700 rounded p-4">
                <h3 className="font-semibold mb-3">メンバーリスト ({(isMember ? members.length : (guild?.members_count || 0))}/5)</h3>
                {!isMember ? (
                  <p className="text-gray-400 text-sm">メンバー人数: {(guild?.members_count || 0)}/5（メンバー名は非公開）</p>
                ) : (
                  members.length === 0 ? (
