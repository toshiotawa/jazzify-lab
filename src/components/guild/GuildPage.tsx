  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setIsMember(member);
        if (g) {
          const pendingIdPromise = fetchMyJoinRequestForGuild(g.id);
          if (member) {
            const [m, per, pendingId] = await Promise.all([
            ...
            <>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {!isMember && (
                      myPendingRequestId ? (
                        ...
                        <button className="btn btn-sm btn-primary" disabled={busy || (guild.members_count >= 5)} onClick={async()=>{ try{ setBusy(true); const id = await requestJoin(guild.id); setMyPendingRequestId(id); alert('参加リクエストを送信しました'); } catch(e:any){ alert(e?.message||'リクエスト送信に失敗しました'); } finally{ setBusy(false); } }}>参加リクエスト</button>
                      )
                    )}
                  </div>
                </div>
              ...
              <div className="bg-slate-800 border border-slate-700 rounded p-4">
                ...
                {isMember ? (
                  ...
                  <p className="text-gray-400 text-sm">メンバー名は非公開です（{guild.members_count}/5）</p>
                )}
              </div>
            </>
          )}
