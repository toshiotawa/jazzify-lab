...
import { resolveCourseAccess, buildBlockStatuses, buildLessonStatuses, isPremiumRank } from '@/utils/lessonAccess';
...
const LessonPage: React.FC = () => {
  ...
  const allowAdminOverride = useMemo(() => isPremiumRank(profile?.rank), [profile?.rank]);
  const { blockStatuses, lessonStatuses } = useMemo(() => {
    const blocks = buildBlockStatuses(lessons, progress, allowAdminOverride);
    const lessonsStatus = buildLessonStatuses(lessons, progress, blocks, allowAdminOverride);
    return {
      blockStatuses: blocks,
      lessonStatuses: lessonsStatus,
    };
  }, [lessons, progress, allowAdminOverride]);
    ...
    try {
      ...
      setCourseUnlockStatus(unlockStatus);
        ...
        if (profile) {
          try {
            const [allProgress, lessonsByCourse] = await Promise.all([
              ...
              Promise.all(sortedCourses.map((course) => fetchLessonsByCourse(course.id))),
            ]);

            const lessonsCountByCourse: Record<string, number> = {};
            sortedCourses.forEach((course, index) => {
              lessonsCountByCourse[course.id] = lessonsByCourse[index].length;
            });

            const completedCountByCourse: Record<string, number> = {};
            allProgress.forEach((progressItem) => {
              if (!completedCountByCourse[progressItem.course_id]) {
                completedCountByCourse[progressItem.course_id] = 0;
              }
              if (progressItem.completed) {
                completedCountByCourse[progressItem.course_id] += 1;
              }
            });

            const progressMap: Record<string, number> = {};
            sortedCourses.forEach((course) => {
              const total = lessonsCountByCourse[course.id] || 0;
        ...
        const firstAccessibleCourse = sortedCourses.find((course) => {
          ...
          const accessResult = resolveCourseAccess({
            course,
            userRank: profile?.rank,
            completedCourseIds: completedCourses,
            adminUnlockFlag: courseUnlockFlag === true ? true : null,
          });
              ...
              <div className="w-full md:w-80 bg-slate-800 border-r border-slate-700 flex flex-col overflow-hidden min-h-0 md:h-full">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', touchAction: 'pan-y' }}>
                    {courses.map((course: Course) => {
                      ...
                      const showAdminBadge = accessResult.unlockedByAdmin;
                      const lockReason = accessResult.reason;
                    ...
                    <div className="flex-1 overflow-y-auto p-6" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', touchAction: 'pan-y' }}>
                        <div className="space-y-6">
                          {Object.entries(groupLessonsByBlock(lessons)).map(([blockNumber, blockLessons]) => {
                            ...
                            const blockStatus = blockStatuses[blockNum];
                            const isBlockUnlocked = blockStatus?.isUnlocked ?? false;
                            const isBlockCompleted = blockStatus?.isCompleted ?? false;
                            const isBlockAdminUnlocked = blockStatus?.unlockedByAdmin ?? false;
                              ...
                              <div key={blockNum} className="bg-slate-800 rounded-lg p-4">
                                <div className="flex items-center justify-between mb-4">
                                  ...
                                  {isBlockCompleted ? (
                                    ...
                                    <span className={isBlockAdminUnlocked ? 'text-emerald-400' : 'text-blue-500'}>
                                      {isBlockAdminUnlocked ? '管理者解放中' : '解放中'}
                                    </span>
                                  ) : (
                                ...
                                <div className="space-y-4">
                                  {blockLessons
                                    ...
                                    .map((lesson) => {
                                      const lessonStatus = lessonStatuses[lesson.id];
                                      const unlocked = lessonStatus?.isUnlocked ?? false;
                                      const completed = lessonStatus?.isCompleted ?? false;
                                      const adminUnlocked = lessonStatus?.unlockedByAdmin ?? false;
                                          ...
                                          className={`p-3 sm:p-4 rounded-lg border-2 transition-all ${
                                            unlocked
                                              ? completed
                                                ...
                                                : adminUnlocked
                                                  ? 'border-emerald-400 bg-emerald-900/20 hover:bg-emerald-900/25 cursor-pointer'
