...
/** Y 間隔ルール */
const Y_TOP_PADDING = 80;            // ブロックヘッダーの上余白
const Y_HEADER_TO_FIRST = 120;       // ヘッダープレート → 1つ目の踊り場
const Y_STAGE_GAP = 150;             // 通常踊り場間（小→小）
const Y_BEFORE_BIG = 200;            // 直前踊り場 → 大踊り場
const Y_DOOR_TO_NEXT_HEADER = 200;   // 扉 → 次ブロックヘッダー
const Y_DOOR_OFFSET_FROM_BIG = -30;  // 大踊り場に対する扉の相対Y（上方・奥行き演出）
