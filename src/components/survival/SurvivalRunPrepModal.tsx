...
interface SurvivalRunPrepModalProps {
  ...
  /** コードラン: モーダルを開いたときの初期オートラン状態 */
  initialAutoRun?: boolean;
  onCancel: () => void;
  /** true = HINT（練習）。コードラン時は第2引数 autoRun = オート操作 */
  onConfirm: (hintMode: boolean, autoRun?: boolean) => void;
}
...
const SurvivalRunPrepModal: React.FC<SurvivalRunPrepModalProps> = ({
  ...
  const showCodeRunControl = stage?.playMode === 'code_run' && variant === 'lesson';
  ...
  return (
    ...
    >
      <div className="max-h-[90dvh] w-full max-w-md overflow-y-auto rounded-2xl border border-amber-500/40 bg-gradient-to-b from-[#1a1025] to-[#0a0610] p-6 text-white shadow-xl">
        ...
        {showCodeRunControl ? (
          <fieldset className="mb-5 space-y-2">
            <legend className="mb-2 text-sm font-semibold text-cyan-200">
              {isEnglishCopy ? 'Character control' : 'キャラ操作'}
            </legend>
            <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-cyan-500/20 bg-black/30 px-3 py-2 hover:bg-white/5">
              <input
                type="radio"
            ...
            <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-cyan-500/20 bg-black/30 px-3 py-2 hover:bg-white/5">
              <input
                ...
                className="radio radio-sm radio-info"
              />
              <span>{isEnglishCopy ? 'Auto' : 'オート'}</span>
            </label>
          </fieldset>
        ) : null}
        ...
        <div className="flex flex-col gap-2 sm:flex-row-reverse sm:justify-end">
          <button
            ...
            onClick={() => onConfirm(
              compositeLocked ? false : hintLocal,
              showCodeRunControl ? autoRunLocal : undefined,
            )}
          >
            {startLabel}
          </button>
