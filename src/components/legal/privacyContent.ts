  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '当社は、Web版の利用状況把握およびマーケティング効果の測定のため、Google LLCが提供するGoogle Analytics 4（GA4）を利用しています。GA4は、Cookieまたはこれに類する識別子を用いて、ページ閲覧数、登録導線の利用状況等の統計情報を収集します。収集された情報はGoogleのプライバシーポリシーに従って管理されます。Googleのプライバシーポリシーは https://policies.google.com/privacy をご参照ください。',
        '広告配信目的のタグ・SDKは導入していません。今後これらを導入する場合は、本ポリシーの改定または別途の通知により公表します。',
      ],
    },
    {
      title: '9. 安全管理措置',
