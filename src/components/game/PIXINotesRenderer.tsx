...
export class PIXINotesRendererInstance {
  ...
  constructor(width: number, height: number) {
    ...
    log.info(
      `🎹 White key width: ${whiteKeyWidth.toFixed(2)}px, Note width: ${this.settings.noteWidth.toFixed(2)}px`,
    );
    ...
    log.debug(
      `🎯 PIXI.js App created - Canvas: ${this.app.view.width}x${this.app.view.height}`,
    );
    ...
    if (width !== this.app.view.width || height !== this.app.view.height) {
      log.warn(
        `🚨 Canvas size mismatch - Expected: ${width}x${height}, Actual: ${this.app.view.width}x${this.app.view.height}`,
      );
    }
  ...
  private resolveCurrentTime(externalTime?: number): number | null {
    if (typeof externalTime === "number" && Number.isFinite(externalTime)) {
      this._currentTime = externalTime;
      return externalTime;
    }
    if (this.timeProvider) {
      const provided = this.timeProvider();
      if (typeof provided === "number" && Number.isFinite(provided)) {
        this._currentTime = provided;
        return provided;
      }
    }
    return Number.isFinite(this._currentTime) ? this._currentTime : null;
  }

  private invalidateSpeedCache(): void {
    this.speedPxPerSec = 0;
  }

  private calculateSpeedPxPerSec(): number {
    ...
    if (totalDistance <= 0) {
      return visualSpeedMultiplier * 60;
    }
    return (totalDistance / baseFallDuration) * visualSpeedMultiplier;
  }

  private getSpeedPxPerSec(): number {
    if (this.speedPxPerSec === 0 || !Number.isFinite(this.speedPxPerSec)) {
      this.speedPxPerSec = this.calculateSpeedPxPerSec();
    }
    return this.speedPxPerSec;
  }

  private syncSpritesWithCurrentTime(requestedTime?: number): void {
    if (this.noteSprites.size === 0 || this.activeNoteLookup.size === 0) {
      return;
    }
    const resolvedTime = this.resolveCurrentTime(requestedTime);
    ...
    this.updateSpritePositions(this.activeNoteLookup, resolvedTime, speed);
  }
  ...
  private setupTickerSystem(): void {
    ...
    this.mainUpdateFunction = (delta: number) => {
      ...
      this.syncSpritesWithCurrentTime();
    };
  ...
  private startUnifiedRendering(): void {
    if (!window.unifiedFrameController) {
      log.warn(
        "⚠️ unifiedFrameController not available, using default PIXI ticker",
      );
      this.app.start();
      return;
    }
    ...
    const renderFrame = () => {
      ...
      // 統合フレーム制御でフレームスキップ判定
      if (
        window.unifiedFrameController.shouldSkipFrame(currentTime, "render")
      ) {
        // フレームをスキップ
  ...
  private generateNoteTextures(): void {
    ...
    log.info(
      `🎯 Generating note textures with size: ${noteWidth}x${noteHeight}`,
    );

    // 白鍵ノーツテクスチャ
    const whiteGraphics = new PIXI.Graphics();
    this.drawNoteShapeToGraphics(
      whiteGraphics,
      "visible",
      false,
      noteWidth,
    ...
    this.noteTextures.blackVisible =
      this.app.renderer.generateTexture(blackGraphics);
    ...
    this.noteTextures.missed =
      this.app.renderer.generateTexture(missedGraphics);
  ...
  private generateLabelTextures(): void {
    ...
    log.info(
      `🎯 Label texture generation completed! Total ABC textures: ${this.labelTextures.abc.size}`,
    );
  }
  ...
  private getSimplifiedDisplayName(noteName: string): string {
    ...
    const simplifiedEnglishName =
      complexToSimpleMap[noteNameWithoutOctave] || noteNameWithoutOctave;
  ...
  private getLabelTexture(noteName: string): PIXI.Texture | null {
    ...
    if (style === "abc") {
      ...
      if (/^[A-G]/.test(noteNameWithoutOctave)) {
        ...
        const englishName = this.convertJapaneseToEnglishNoteName(
    ...
    if (!texture || texture === PIXI.Texture.EMPTY) {
      log.warn(
        `⚠️ getLabelTexture: No texture found for "${noteNameWithoutOctave}" (style: ${style})`,
      );
      return null;
    }
   ...
   */
  private drawNoteShapeToGraphics(
    ...
    if (state === "visible") {
      if (isBlackKey) {
        ...
        for (let i = 0; i < steps; i++) {
          ...
          const r1 = 0x16,
            g1 = 0x21,
            b1 = 0x3e; // topColor RGB
          const r2 = 0x0f,
            g2 = 0x34,
            b2 = 0x60; // bottomColor RGB
  ...
  private setupPiano(): void {
    ...
    for (let note = minNote; note <= maxNote; note++) {
      if (!this.isBlackKey(note)) {
        const whiteKey = this.createWhiteKey(
    ...
    for (let note = minNote; note <= maxNote; note++) {
      if (this.isBlackKey(note)) {
        const position = this.calculateBlackKeyPosition(
          note,
          minNote,
          maxNote,
          totalWhiteKeys,
  ...
  private isKeyHighlighted(midiNote: number): boolean {
    return (
      this.highlightedKeys.has(midiNote) ||
      this.guideHighlightedKeys.has(midiNote)
    );
  }

  private calculateBlackKeyPosition(
    note: number,
    minNote: number,
    _maxNote: number,
   ...
   * 黒鍵のハイライト状態を再描画
   */
  private redrawBlackKeyHighlight(
    keySprite: PIXI.Graphics,
    highlighted: boolean,
    midiNote?: number,
  ): void {
    keySprite.clear();
    ...
    const whiteKeyWidth =
      this.app.screen.width / this.calculateTotalWhiteKeys();
    const blackKeyWidthRatio = 0.8;
    ...
    if (highlighted) {
      const isActive =
        midiNote !== undefined && this.highlightedKeys.has(midiNote);
      const baseColor = isActive
        ? this.settings.colors.activeKey
        : this.settings.colors.guideKey;
  ...
  updateNotes(activeNotes: ActiveNote[], currentTime?: number): void {
    const effectiveTime = this.resolveCurrentTime(currentTime);
    ...
    const jumpThreshold =
      PIXI_LOOKAHEAD_SECONDS > 0 ? PIXI_LOOKAHEAD_SECONDS * 0.5 : 1;
    ...
    while (
      this.nextNoteIndex < this.allNotes.length &&
    ...
    this.syncSpritesWithCurrentTime(effectiveTime);
  ...
  private updateSpritePositions(
    ...
    for (const [noteId, sprite] of this.noteSprites) {
      ...
      if (suppliedY !== undefined) {
        ...
        // フォールバック: 自前計算
        newY =
          this.settings.hitLineY - (note.time - currentTime) * speedPxPerSec;
      }
          ...
          if (this.settings.simpleDisplayMode) {
            ...
            if (note.noteName) {
              if (this.settings.noteNameStyle === "solfege") {
                ...
                // 英語簡易表示
                newNoteName = this.getEnglishSimplifiedDisplayName(
                  note.noteName,
                );
              }
            } else {
  ...
  private updateSpriteStates(activeNoteLookup: Map<string, ActiveNote>): void {
    ...
    for (const [noteId, sprite] of this.noteSprites) {
      ...
      if (
        sprite.noteData.state !== note.state ||
        sprite.noteData.noteName !== note.noteName
      ) {
        // 🚀 ヒット系判定時は即座処理
    ...
    if (this.settings.simpleDisplayMode) {
      ...
      if (note.noteName) {
        if (this.settings.noteNameStyle === "solfege") {
          ...
          // 英語簡易表示
          noteNameForLabel = this.getEnglishSimplifiedDisplayName(
            note.noteName,
          );
        }
      } else {
      ...
      try {
        ...
        if (labelTexture) {
          ...
          try {
            ...
            log.error(
              `❌ Failed to add label to container for "${noteNameForLabel}":`,
              containerError,
            );
            label.destroy();
            label = undefined;
        ...
        log.error(
          `❌ Error creating label sprite for "${noteNameForLabel}":`,
          error,
        );
        label = undefined;
      }
  ...
  private updateNoteState(noteSprite: NoteSprite, note: ActiveNote): void {
    ...
    if (
      note.crossingLogged &&
      !noteSprite.noteData.crossingLogged &&
      this.settings.practiceGuide !== "off"
    ) {
  ...
  private updateHitEffects(deltaMs: number): void {
    ...
    for (const effectData of this.activeHitEffects) {
      ...
      const progress = Math.min(
        1,
        effectData.elapsed / this.hitEffectDurationMs,
      );
  ...
  private redrawCircleSprites(circleContainer: PIXI.Container): void {
    const outerCircle = circleContainer.getChildByName(
      "outerCircle",
    ) as PIXI.Graphics | null;
    const middleCircle = circleContainer.getChildByName(
      "middleCircle",
    ) as PIXI.Graphics | null;
    const innerCircle = circleContainer.getChildByName(
      "innerCircle",
    ) as PIXI.Graphics | null;
  ...
  private getStateColor(state: ActiveNote["state"], pitch?: number): number {
    switch (state) {
      case "visible":
        if (
          pitch !== undefined &&
          this.isBlackKey(pitch + this.settings.transpose)
        ) {
  ...
  private pitchToX(pitch: number): number {
    ...
    if (this.isBlackKey(effectivePitch)) {
      ...
      return this.calculateBlackKeyPosition(
        effectivePitch,
        minNote,
        maxNote,
        totalWhiteKeys,
  ...
  updateSettings(newSettings: Partial<RendererSettings>): void {
    ...
    if (!this.app || (this.app as any)._destroyed || !this.app.screen) {
      log.warn(
        "PIXINotesRendererInstance.updateSettings: renderer already destroyed, skipping",
      );
      return;
    }
    ...
    const prevNoteSpeed = this.settings.noteSpeed;
    ...
    if (
      newSettings.pianoHeight !== undefined &&
      newSettings.pianoHeight !== prevPianoHeight
    ) {
      // 新しい判定ラインYを計算
    ...
    if (
      newSettings.noteSpeed !== undefined &&
      newSettings.noteSpeed !== prevNoteSpeed
    ) {
      this.invalidateSpeedCache();
    }
    ...
    if (
      (newSettings.noteNameStyle !== undefined &&
        newSettings.noteNameStyle !== prevNoteNameStyle) ||
      (newSettings.simpleDisplayMode !== undefined &&
        newSettings.simpleDisplayMode !== prevSimpleDisplayMode)
    ) {
      // 鍵盤の音名表示を更新（鍵盤を再描画）
      ...
      this.noteSprites.forEach((noteSprite) => {
        ...
        if (this.settings.simpleDisplayMode) {
          ...
          if (noteSprite.noteData.noteName) {
            if (this.settings.noteNameStyle === "solfege") {
              // ドレミ簡易表示
              noteName = this.getSimplifiedDisplayName(
                noteSprite.noteData.noteName,
              );
            } else {
              // 英語簡易表示
              noteName = this.getEnglishSimplifiedDisplayName(
                noteSprite.noteData.noteName,
              );
            }
          } else {
          ...
          noteName =
            noteSprite.noteData.noteName ||
            this.getMidiNoteName(effectivePitch);
        }
    ...
    if (
      newSettings.transposingInstrument !== undefined &&
      newSettings.transposingInstrument !== prevTransposingInstrument
    ) {
      // 鍵盤の音名表示を更新（鍵盤を再描画）
    ...
    if (
      newSettings.transpose !== undefined &&
      newSettings.transpose !== prevTranspose
    ) {
      // 全てのノートスプライトを即座に更新
      this.noteSprites.forEach((noteSprite) => {
        const pitch = noteSprite.noteData.pitch;
        ...
        if (this.settings.simpleDisplayMode) {
          ...
          if (noteSprite.noteData.noteName) {
            if (this.settings.noteNameStyle === "solfege") {
              // ドレミ簡易表示
              displayNoteName = this.getSimplifiedDisplayName(
                noteSprite.noteData.noteName,
              );
            } else {
              // 英語簡易表示
              displayNoteName = this.getEnglishSimplifiedDisplayName(
                noteSprite.noteData.noteName,
              );
            }
          } else {
        ...
        const isBlackNote = this.isBlackKey(
          noteData.pitch + this.settings.transpose,
        );
   ...
   */
  private fadeOutLater(
    display: PIXI.DisplayObject & { alpha: number },
    duration: number,
    onComplete?: () => void,
  ): void {
  ...
  private applyKeyHighlightVisual(
    midiNote: number,
    highlighted: boolean,
  ): void {
    const keySprite = this.pianoSprites.get(midiNote);
    ...
    if (this.isBlackKey(midiNote)) {
      ...
      (keySprite as any).tint = isActive
        ? this.settings.colors.activeKey
        : this.settings.colors.guideKey;
    }
  }
  ...
  public triggerKeyPressEffect(midiNote: number): void {
    ...
    // 早押し時の誤エフェクト防止
    const distanceToHitLine = Math.abs(
      targetSprite.sprite.y - this.settings.hitLineY,
    );
  ...
  resize(width: number, height: number): void {
    ...
    if (Math.abs(newNoteWidth - this.settings.noteWidth) > 0.1) {
      // 誤差を考慮
      this.settings.noteWidth = newNoteWidth;
      log.info(
        `🔄 Regenerating note textures with new width: ${newNoteWidth.toFixed(2)}px`,
      );
      ...
      this.noteSprites.forEach((ns) => {
        const isBlack = this.isBlackKey(
          ns.noteData.pitch + this.settings.transpose,
        );
...
export const PIXINotesRenderer: React.FC<PIXINotesRendererProps> = ({
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (el) {
        ...
        log.debug?.(
          "ℹ️ Skipping fade-in: containerRef is null (likely unmounted)",
        );
      }
    });
  ...
  useEffect(() => {
    if (rendererRef.current) {
      rendererRef.current.setTimeProvider(timeProvider);
    }
  }, [timeProvider]);

  useEffect(() => {
    if (rendererRef.current) {
      const resolvedTime = timeProvider?.();
      rendererRef.current.updateNotes(activeNotes, resolvedTime);
    }
  }, [activeNotes, timeProvider]);
  ...
  return (
    <div
      ref={containerRef}
      className={cn("custom-pixi-scrollbar", className)}
      style={{
        width,
        height: actualHeight,
