...
const SCORE_PADDING_LEFT = PLAYHEAD_OFFSET_PX + 40;
const SCORE_PADDING_RIGHT = PLAYHEAD_OFFSET_PX + 260;
...
const SheetMusicDisplay: React.FC<SheetMusicDisplayProps> = ({ className = '' }) => {
  ...
  const scaleFactorRef = useRef<number>(10); // デフォルトは以前のマジックナンバー
    ...
    const { currentTime, isPlaying, notes, musicXml, settings, currentSongId } = useGameSelector((s) => ({
    ...
    }, [currentSongId, releaseProgrammaticScroll]);
  ...
  const createTimeMapping = useCallback(() => {
    ...
    lastScrollXRef.current = 0;
      ...
      useEffect(() => {
        ...
        const container = scrollContainerRef.current;
        if (!container || !isAutoFollowEnabled) {
          return;
        }
        ...
        const paddedX = targetEntry.xPosition + SCORE_PADDING_LEFT;
        const rawScroll = Math.max(0, paddedX - PLAYHEAD_OFFSET_PX);
        const maxScroll = Math.max(0, container.scrollWidth - container.clientWidth);
        const nextScrollLeft = Math.min(rawScroll, maxScroll);
  
        const needsIndexUpdate = activeIndex !== lastRenderedIndexRef.current;
        const needsScrollUpdate = Math.abs(nextScrollLeft - lastScrollXRef.current) > 0.5;
  
        if (needsIndexUpdate || needsScrollUpdate) {
          isProgrammaticScrollRef.current = true;
          container.scrollLeft = nextScrollLeft;
          releaseProgrammaticScroll();
          lastRenderedIndexRef.current = activeIndex;
          lastScrollXRef.current = nextScrollLeft;
        }
      }, [currentTime, isPlaying, notes, shouldRenderSheet, isAutoFollowEnabled, releaseProgrammaticScroll]);
      ...
      useEffect(() => {
        ...
        const handleScroll = () => {
          if (isProgrammaticScrollRef.current || !autoFollowEnabledRef.current) {
            return;
          }
    ...
    >
        {!isAutoFollowEnabled && (
          <div className="absolute top-3 right-4 z-20 flex justify-end">
            <button
              type="button"
              className="px-3 py-1 text-xs font-semibold rounded-full bg-slate-900/80 text-slate-100 shadow hover:bg-slate-800 transition-colors"
              onClick={() => setIsAutoFollowEnabled(true)}
            >
              自動追従を再開
            </button>
          </div>
      ...
      <div 
        className="absolute top-0 bottom-0 w-0.5 bg-red-500 z-10 pointer-events-none"
          style={{ left: `${PLAYHEAD_OFFSET_PX}px` }}
      />
      ...
      <div className="relative h-full pt-8 pb-4">
        ...
        {/* OSMDレンダリング用コンテナ */}
            <div 
              ...
              style={{ 
                paddingLeft: `${SCORE_PADDING_LEFT}px`,
                paddingRight: `${SCORE_PADDING_RIGHT}px`
              }}
            >
          <div 
            ref={containerRef} 
