...
const SheetMusicDisplay: React.FC<SheetMusicDisplayProps> = ({ className = '' }) => {
  ...
  // OSMD終端対応: 譜面幅とプレイヘッド位置
  const sheetWidthRef = useRef<number>(0); // 譜面全体のpx幅（OSMDのBoundingBox*scale）
  const [playheadLeft, setPlayheadLeft] = useState<number>(120); // プレイヘッドのX(px)
  ...
  const loadAndRenderSheet = useCallback(async () => {
    ...
    setError(null);
      ...
      // レンダリング後に正確なスケールファクターを計算
        ...
        // 譜面幅を保存
        if (boundingBox && typeof boundingBox.width === 'number') {
          sheetWidthRef.current = boundingBox.width * scaleFactorRef.current;
          // ラッパの見かけ幅を合わせると座標計算が安定
  ...
  // 音符の時刻とX座標のマッピングを作成
    ...
    // currentTimeが変更されるたびにスクロール位置を更新（終端対応版）
    useEffect(() => {
      const mapping = timeMappingRef.current;
      ...
      prevTimeRef.current = currentTime;
    }, [currentTime, isPlaying, shouldRenderSheet, getXForTime, playheadLeft]);
