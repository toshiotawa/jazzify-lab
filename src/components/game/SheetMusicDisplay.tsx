  ...
  // 音符の時刻とX座標のマッピングを作成
    ...
    // currentTimeが変更されるたびにスクロール位置を更新（音符単位でジャンプ）
      useEffect(() => {
        ...
        const isPastLastNote = rawIndex >= mapping.length - 1 && lastEntry !== undefined;

        if (isPastLastNote && lastEntry && currentTimeMs > lastEntry.timeMs && scrollContainer) {
      ...
      }, [currentTime, currentSong, isPlaying, notes, shouldRenderSheet]);
