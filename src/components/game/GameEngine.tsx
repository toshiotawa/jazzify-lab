...
export const GameEngineComponent: React.FC<GameEngineComponentProps> = ({ 
  ...
  // GameEngine と updateTime に渡すための AudioContext ベースのタイムスタンプ
    ...
    const engineActiveNotesRef = useRef<ActiveNote[]>(useGameStore.getState().engineActiveNotes);
    
    useEffect(() => {
      currentTimeRef.current = useGameStore.getState().currentTime;
      const unsubscribe = useGameStore.subscribe(
        (state) => state.currentTime,
        (newTime) => {
          currentTimeRef.current = newTime;
          if (!useGameStore.getState().isPlaying && pixiRendererRef.current) {
            pixiRendererRef.current.updateCurrentTime(newTime);
          }
        }
      );
      return unsubscribe;
    ...
    useEffect(() => {
      engineActiveNotesRef.current = useGameStore.getState().engineActiveNotes;
      const unsubscribe = useGameStore.subscribe(
        (state) => state.engineActiveNotes,
        (notes) => {
          engineActiveNotesRef.current = notes;
  ...
  useEffect(() => {
    ...
    const resolvePlaybackTime = () => {
      ...
      if (hasAudio && audioCtx && audio && !audio.paused) {
    ...
    const scheduleSongCompletionCheck = (time: number) => {
      const runCheck = () => {
    ...
    const runTimeLoop = () => {
      ...
      scheduleSongCompletionCheck(newTime);
      animationFrameRef.current = requestAnimationFrame(runTimeLoop);
    };
    ...
    useEffect(() => {
      ...
      const syncPlayback = (targetTime: number) => {
        ...
        if (hasAudio) {
          ...
          const timeDiff = Math.abs(audioTime - safeTime);
          if (timeDiff > 0.3) {
            if (audioRef.current) {
          ...
          if (timeDiff > 0.3) {
            ...
            updateTime(safeTime);
            devLog.debug(`🔄 GameEngine (audio off) synced to ${safeTime.toFixed(2)}s`);
          }
        }
      };
      ...
      return unsubscribe;
    }, [audioLoaded, gameEngine, settings.playbackSpeed, currentSong?.duration]);
  ...
  const handlePixiReady = useCallback((renderer: PIXINotesRendererInstance | null) => {
    ...
    renderer.updateNotes(engineActiveNotesRef.current, currentTimeRef.current);
    renderer.updateCurrentTime(currentTimeRef.current);
