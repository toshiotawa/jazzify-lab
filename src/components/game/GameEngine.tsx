...
export const GameEngineComponent: React.FC<GameEngineComponentProps> = ({ 
  ...
  useEffect(() => {
    ...
    hasAudioRef.current = hasAudioFile;
    shouldUseTonePlayerRef.current = hasAudioFile && isIOSDevice;
  ...
  }, [currentSong?.audioFile, currentSong?.duration, isIOSDevice]);
    ...
    useEffect(() => {
      ...
      const audioDuration = audioDurationRef.current || currentSong.duration || 0;
      const audioComplete = !hasAudioRef.current || audioDuration === 0 || currentTime >= audioDuration - 0.05;
      if (score.totalNotes > 0 && judgedNotes >= score.totalNotes && audioComplete && !resultModalOpen) {
        stageRunStateRef.current = 'completed';
        pause();
        openResultModal();
      }
    }, [mode, currentSong?.id, score.goodCount, score.missCount, score.totalNotes, pause, openResultModal, resultModalOpen, currentTime]);
  ...
  const shouldUseTonePlayerRef = useRef(false);
  const usingTonePlayerRef = useRef(false);
  ...
  const baseOffsetRef = useRef<number>(0); // currentTime = audioCtx.time - baseOffset
    ...
    const playbackRateRef = useRef(settings.playbackSpeed);
    ...
    const applyPlaybackRate = useCallback((rate: number) => {
      if (!shouldUseTonePlayerRef.current && audioRef.current) {
  ...
  // 楽曲読み込み時の音声設定
    useEffect(() => {
      ...
      const handleLoadedMetadata = () => {
          ...
          if (!shouldUseTonePlayerRef.current) {
            setAudioLoaded(true);
          }
          if (!shouldUseTonePlayerRef.current) {
    ...
    useEffect(() => {
      ...
      const shouldUseTone = shouldUseTonePlayerRef.current && Boolean(audioFile);
      if (!shouldUseTone) {
      ...
      let disposed = false;
      setAudioLoaded(false);
      const prepareTonePlayer = async (): Promise<void> => {
        try {
          await Tone.start();
          ...
          if (typeof (Tone as unknown as { setContext?: (ctx: AudioContext) => void }).setContext === 'function') {
          ...
          toneGainRef.current?.disconnect();
          toneGainRef.current?.dispose();
          toneGainRef.current = new Tone.Gain(musicVolumeRef.current).toDestination();
    ...
    }, [currentSong?.audioFile, isIOSDevice]);
  ...
  // 再生状態同期
    useEffect(() => {
      ...
      const run = async () => {
        if (isPlaying) {
          const hasAudio = hasAudioRef.current && audioLoaded;
          const useTonePlayback = hasAudio && shouldUseTonePlayerRef.current && tonePlayerRef.current;

          if (hasAudio && useTonePlayback && tonePlayerRef.current) {
            const audioContext = audioContextRef.current ?? new (window.AudioContext || (window as any).webkitAudioContext)();
            audioContextRef.current = audioContext;
            usingTonePlayerRef.current = true;
            if (audioRef.current) {
              audioRef.current.pause();
            }
            await audioContext.resume().catch((error) => log.warn('AudioContext resume failed', error));
            const syncTime = Math.max(0, currentTime);
            const realTimeElapsed = syncTime / Math.max(0.1, playbackRateRef.current);
            ...
            if (shouldUsePitchShift) {
              ...
              if (toneGainRef.current && pitchShiftRef.current) {
                ...
                pitchShiftRef.current.connect(toneGainRef.current);
              }
              ...
              if (toneGainRef.current) {
                tonePlayerRef.current.disconnect();
                tonePlayerRef.current.connect(toneGainRef.current);
              }
            }
            ...
            tonePlayerRef.current.stop();
            tonePlayerRef.current.start(undefined, syncTime);
          } else if (hasAudio && audioRef.current) {
            usingTonePlayerRef.current = false;
            const audio = audioRef.current;
            if (!audioContextRef.current) {
              audioContextRef.current = new (window.AudioContext || (window as any).webkitAudioContext)();
            }
          ...
          if (usingTonePlayerRef.current && tonePlayerRef.current) {
            tonePlayerRef.current.stop();
          }
          if (audioRef.current) {
            audioRef.current.pause();
    ...
    }, [isPlaying, audioLoaded, gameEngine, settings.transpose, applyPlaybackRate, isIOSDevice]);
  ...
  // 音量変更の同期
    ...
    // 再生スピード変更の同期
    useEffect(() => {
      playbackRateRef.current = settings.playbackSpeed;
    ...
    useEffect(() => {
      if (audioContextRef.current && gameEngine) {
        ...
        if (hasAudio && useTonePlayback && tonePlayerRef.current) {
          const audioTime = (audioContextRef.current.currentTime - baseOffsetRef.current) * playbackRateRef.current;
          const timeDiff = Math.abs(audioTime - currentTime);
            ...
            const realTimeElapsed = safeTime / Math.max(0.1, playbackRateRef.current);
            baseOffsetRef.current = audioContextRef.current.currentTime - realTimeElapsed;
            gameEngine.seek(safeTime);
            updateTime(safeTime);
            devLog.debug(`🔄 Tone & GameEngine synced to ${safeTime.toFixed(2)}s`);
          }
        } else if (hasAudio && audioRef.current) {
          const audioTime = (audioContextRef.current.currentTime - baseOffsetRef.current) * playbackRateRef.current;
          const timeDiff = Math.abs(audioTime - currentTime);
          ...
          const timeDiff = Math.abs((audioContextRef.current.currentTime - baseOffsetRef.current) * playbackRateRef.current - currentTime);
          if (timeDiff > 0.3) {
            const safeTime = Math.max(0, Math.min(currentTime, currentSong?.duration || currentTime));
            const realTimeElapsed = safeTime / Math.max(0.1, playbackRateRef.current);
            baseOffsetRef.current = audioContextRef.current.currentTime - realTimeElapsed;
    ...
    }, [currentTime, audioLoaded, gameEngine, updateTime, isPlaying]);
