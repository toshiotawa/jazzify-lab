...
export const GameEngineComponent: React.FC<GameEngineComponentProps> = ({ 
  ...
  const startTimeSync = useCallback(() => {
    ...
    const tick = () => {
      const { isPlaying: storePlaying } = useGameStore.getState();
      if (!storePlaying) {
        animationFrameRef.current = null;
        return;
      }

      const newTime = getPlaybackTime();
      updateTime(newTime);
    ...
    animationFrameRef.current = window.requestAnimationFrame(tick);
  }, [currentSong?.duration, getPlaybackTime, mode, openResultModal, stop, stopTimeSync, updateTime]);
  ...
  useEffect(() => {
    ...
    pixiRenderer.setTimeProvider(() => getPlaybackTime());
    return () => {
      pixiRenderer.setTimeProvider(undefined);
    };
  }, [pixiRenderer, getPlaybackTime]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = useGameStore.subscribe(
      ...
      (nextTime) => {
        ...
        const hasAudio =
          Boolean(currentSong?.audioFile && currentSong.audioFile.trim() !== '' && audioRef.current && audioLoaded);
        const songDuration = currentSong?.duration ?? nextTime;

        if (hasAudio) {
          const audioTime = (audioCtx.currentTime - baseOffsetRef.current) * settings.playbackSpeed;
          const timeDiff = Math.abs(audioTime - nextTime);
          if (timeDiff > 0.3) {
            const safeTime = Math.max(0, Math.min(nextTime, songDuration));
            if (audioRef.current) {
          ...
          const timeDiff = Math.abs((audioCtx.currentTime - baseOffsetRef.current) * settings.playbackSpeed - nextTime);
          if (timeDiff > 0.3) {
            const safeTime = Math.max(0, Math.min(nextTime, songDuration));
            const realTimeElapsed = safeTime / settings.playbackSpeed;
            baseOffsetRef.current = audioCtx.currentTime - realTimeElapsed;
  ...
  }, [audioLoaded, currentSong, gameEngine, settings.playbackSpeed]);
