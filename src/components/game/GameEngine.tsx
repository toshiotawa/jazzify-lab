...
export const GameEngineComponent: React.FC<GameEngineComponentProps> = ({ 
  ...
  // GameEngine と updateTime に渡すための AudioContext ベースのタイムスタンプ
    ...
    useEffect(() => {
      ...
      if (currentState.mode !== 'performance' || !currentSongId) {
        return;
      }
    ...
    }, [currentSongId, pause, openResultModal, resultModalOpen, hasPlaybackFinished]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
        ...
        const syncTime = Math.max(0, useGameStore.getState().currentTime);
  ...
  useEffect(() => {
    ...
    if (audioRef.current) {
      ...
      if (audioContextRef.current && isPlaying) {
        const newElapsedReal = useGameStore.getState().currentTime / settings.playbackSpeed;
  ...
  useEffect(() => {
    ...
    // currentTimeをsubscribeで監視し、DOM更新を回避
    const unsubscribe = useGameStore.subscribe(
      (state) => state.currentTime,
      (currentTime) => {
        const currentState = useGameStore.getState();
        const hasAudio = currentState.currentSong?.audioFile && currentState.currentSong.audioFile.trim() !== '' && audioRef.current && audioLoaded;
        
        if (hasAudio) {
          // 音声ありの場合: 音声とゲームエンジンの同期
          if (!audioContextRef.current) return;
          const audioTime = (audioContextRef.current.currentTime - baseOffsetRef.current) * currentState.settings.playbackSpeed;
          ...
          if (timeDiff > 0.3) {
            ...
            // オフセット再計算（再生速度を考慮）
            if (audioContextRef.current) {
              const realTimeElapsed = safeTime / currentState.settings.playbackSpeed;
            ...
            useGameStore.getState().updateTime(safeTime);
          ...
          if (!audioContextRef.current) return;
          const timeDiff = Math.abs((audioContextRef.current.currentTime - baseOffsetRef.current) * currentState.settings.playbackSpeed - currentTime);
          if (timeDiff > 0.3) {
            const safeTime = Math.max(0, Math.min(currentTime, currentSongDuration ?? currentTime));
    ...
    return unsubscribe;
  }, [audioLoaded, gameEngine, audioElementKey, currentSongDuration, hasAudioTrack]);
  ...
  // コード情報の更新（Canvasに統合）
  useEffect(() => {
    if (!pixiRenderer || chords.length === 0) {
      return;
    }

    // currentTimeの変更を監視してコードを更新
    const unsubscribe = useGameStore.subscribe(
      (state) => state.currentTime,
      (currentTime) => {
        const chord = chords.find(
          (c) => currentTime >= c.startTime && (c.endTime === undefined || currentTime < c.endTime)
        );
        pixiRenderer.updateChord(chord ? chord.symbol.displayText : '');
      }
    );
