...
export const GameEngineComponent: React.FC<GameEngineComponentProps> = ({ 
  ...
  const scoreStats = useGameSelector((state) => ({
  ...
  const getVisualCurrentTime = useCallback((): number => {
    const audio = audioRef.current;
    const audioCtx = audioContextRef.current;
    const hasAudioFile =
      Boolean(currentSong?.audioFile && currentSong.audioFile.trim() !== '') &&
      Boolean(audio) &&
      audioLoaded;

    if (hasAudioFile && audio) {
  ...
  }, [audioLoaded, currentSong?.audioFile, settings.playbackSpeed]);

  const maybeStopAtSongEnd = useCallback(
    (time: number) => {
      const songDuration = useGameStore.getState().currentSong?.duration || 0;
      if (songDuration === 0 || time < songDuration) {
        return;
      }
      stop();
  ...
  const runTimeSync = useCallback(() => {
    ...
    const newTime = getVisualCurrentTime();
    updateTime(newTime);
    maybeStopAtSongEnd(newTime);

    animationFrameRef.current = requestAnimationFrame(runTimeSync);
  }, [getVisualCurrentTime, maybeStopAtSongEnd, updateTime]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = useGameStore.subscribe(
      ...
      (storeTime) => {
        ...
        if (hasAudio) {
          ...
          const timeDiff = Math.abs(audioTime - storeTime);
          if (timeDiff > 0.3) {
            const duration = currentSong?.duration ?? storeTime;
            const safeTime = Math.max(0, Math.min(storeTime, duration));
            if (audioRef.current) {
        ...
        const audioLessTime = (audioContext.currentTime - baseOffsetRef.current) * settings.playbackSpeed;
        const timeDiff = Math.abs(audioLessTime - storeTime);
        if (timeDiff > 0.3) {
          const duration = currentSong?.duration ?? storeTime;
          const safeTime = Math.max(0, Math.min(storeTime, duration));
          const realTimeElapsed = safeTime / settings.playbackSpeed;
    ...
    return unsubscribe;
  }, [audioLoaded, gameEngine, settings.playbackSpeed, currentSong?.audioFile, currentSong?.duration]);
  ...
  // ================= PIXI.js レンダラー準備完了ハンドラー =================
    ...
    useEffect(() => {
      ...
      pixiRenderer.setTimeProvider(getVisualCurrentTime);
      return () => {
        pixiRenderer.setTimeProvider(undefined);
      };
    }, [pixiRenderer, getVisualCurrentTime]);
    ...
    <div className={cn("h-full w-full flex flex-col", className)}>
      ...
      >
        {/* GOOD / MISS オーバーレイ */}
          ...
          <span className="text-green-400">✓ {scoreStats.goodCount}</span>
          <span className="mx-3 text-gray-500">|</span>
          <span className="text-red-400">× {scoreStats.missCount}</span>
          </div>
          )}
