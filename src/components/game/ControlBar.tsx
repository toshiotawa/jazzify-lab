...
const ControlBar: React.FC = () => {
  ...
  const handlePointerDown = (e: React.PointerEvent, type: 'seek' | 'A' | 'B') => {
    ...
    if (mode === 'performance' && type !== 'seek') return; // ステージモードではループ操作禁止
            ...
            <>
              ...
              <div className="flex items-center space-x-2 ml-4 border-l border-gray-700 pl-4">
                ...
                <div className="flex items-center space-x-1 text-xs">
                   <button onClick={handleSetAStart} className={`px-1.5 py-0.5 rounded ${abRepeat.startTime !== null ? 'bg-green-700 text-white' : 'bg-gray-700 text-gray-300'}`}>A</button>
                   {abRepeat.startTime !== null && <button onClick={handleClearA} className="text-gray-400 hover:text-red-400"><FaTimes size={10}/></button>}
                   <span className="text-gray-600">|</span>
                   <button onClick={handleSetBEnd} className={`px-1.5 py-0.5 rounded ${abRepeat.endTime !== null ? 'bg-red-700 text-white' : 'bg-gray-700 text-gray-300'}`}>B</button>
                   {abRepeat.endTime !== null && <button onClick={handleClearB} className="text-gray-400 hover:text-red-400"><FaTimes size={10}/></button>}
                </div>
              </div>
