...
final class EarTrainingChordOSMDBattleController: ObservableObject, EarTrainingOsmdPlayheadBinding {
    ...
    func handleNoteOn(midi: Int, velocity: Int, playAudio: Bool, midiHostTime: UInt64?) {
        ...
        if allowPitchClass {
            ...
            if ExpectedPitchCandidateCollectors.isChordOsmdWaitingForSamePitchRepeat(
                ...
                runtimeAt: runtimeAt,
                targetMidisAt: { [self] index in Array(targets[index].midiCounts.keys) },
                earlySec: earlySec,
                lateSec: lateSec
            ) {
                let minIntervalMs = ExpectedPitchCandidateCollectors.resolveChordOsmdSamePitchRepeatMinIntervalMs(
                    targetCount: targets.count,
                    phraseTimeSec: phraseTime,
                    judgedTargetTimeSec: { [self] index in
                        resolveCalibratedTargetTimeSec(targets[index].targetTimeSec)
    ...
    private func handleAudioTimeUpdate(currentTime: Double) {
        ...
        if NoteInputPreferences.inputMethod == .voice {
            ...
            let nextCandidates = ExpectedPitchCandidateCollectors.collectChordOsmd(
                ...
                earlySec: earlySec,
                lateSec: lateSec,
                targetMidisAt: { [self] index in
                    Array(targets[index].midiCounts.keys)
                }
            )
    ...
    private func openJudgmentWindows(at time: Double) {
        ...
        while nextActiveTargetIndex < targets.count {
            ...
            guard time >= resolveCalibratedTargetTimeSec(target.targetTimeSec) - judgmentWindowEarly else { break }
    ...
    private func compactActiveTargets(currentTime: Double? = nil) {
        ...
        for index in activeTargetIndices {
            ...
            if let currentTime, currentTime > resolveCalibratedTargetTimeSec(target.targetTimeSec) + judgmentWindowLate {
                continue
            }
    ...
    private struct RhythmTarget {
        ...
        func canConsume(midi: Int, allowPitchClass: Bool) -> Bool {
            if (remainingMidiCounts[midi] ?? 0) > 0 { return true }
            guard allowPitchClass else { return false }
            let inputPc = ((midi % 12) + 12) % 12
            for (targetMidi, count) in remainingMidiCounts where count > 0 {
                if ((targetMidi % 12) + 12) % 12 == inputPc { return true }
            }
            return false
        }
