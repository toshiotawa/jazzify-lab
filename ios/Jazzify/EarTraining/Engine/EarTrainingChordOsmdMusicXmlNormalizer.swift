...
enum EarTrainingChordOsmdMusicXmlNormalizer {
    ...
    /// 削除小節の `<attributes>` は新しい先頭小節へ引き継ぐ。
    static func stripOsmdCountInMeasuresFromMusicXml(
        ...
        func stripFromPart(_ element: ChordOsmdXmlElement) {
            if element.name == "part" {
                let measures = measureElements(from: element)
                guard !measures.isEmpty else { return }

                var collected: [String: ChordOsmdXmlElement] = [:]
                let removeCount = min(countInMeasures, measures.count)
                for index in 0 ..< removeCount {
                    collectAttributesChildrenFromMeasure(measures[index], into: &collected)
                }

                for index in 0 ..< removeCount {
                    guard let measureIdx = element.children.firstIndex(where: { child in
                        if case let .element(el) = child, el === measures[index] {
                            return true
                        }
                        return false
                    }) else {
                    ...
                    element.children.remove(at: measureIdx)
                    changed = true
                }

                if let newFirstMeasure = measureElements(from: element).first,
                   mergeCollectedAttributesIntoMeasure(measure: newFirstMeasure, collected: collected)
                {
                    changed = true
                }
                return
    ...
    private static let attributeChildCanonicalOrder = ["divisions", "key", "time", "staves", "clef"]

    private static func attributesChildKey(_ element: ChordOsmdXmlElement) -> String {
        let number = attribute(named: "number", on: element)?
    ...
    private static func canonicalAttributesChildOrder(_ localName: String) -> Int {
        if let idx = attributeChildCanonicalOrder.firstIndex(of: localName) {
            return idx
        }
        return attributeChildCanonicalOrder.count + Int(localName.first?.asciiValue ?? 0)
    }

    private static func measureElements(from part: ChordOsmdXmlElement) -> [ChordOsmdXmlElement] {
        part.children.compactMap { child in
    ...
    private static func collectAttributesChildrenFromMeasure(
        _ measure: ChordOsmdXmlElement,
        into collected: inout [String: ChordOsmdXmlElement]
    ) {
        guard let attributes = directChild(measure, localName: "attributes") else { return }
        for ch in attributes.children {
            guard case let .element(el) = ch else { continue }
            collected[attributesChildKey(el)] = el
        }
    }

    private static func newAttributesInsertIndex(in measure: ChordOsmdXmlElement) -> Int {
        var insertAt = 0
        for (idx, ch) in measure.children.enumerated() {
            if case let .element(el) = ch, el.name == "print" {
                insertAt = idx + 1
            } else if case .element = ch {
                break
            }
        }
        return insertAt
    ...
    private static func insertAttributesChildInCanonicalOrder(
        attributes: ChordOsmdXmlElement,
        newChild: ChordOsmdXmlElement
    ) {
        let newKey = attributesChildKey(newChild)
        let newOrder = canonicalAttributesChildOrder(newChild.name)
        for (idx, ch) in attributes.children.enumerated() {
            guard case let .element(el) = ch else { continue }
            let childOrder = canonicalAttributesChildOrder(el.name)
            let childKey = attributesChildKey(el)
            if childOrder > newOrder || (childOrder == newOrder && childKey > newKey) {
                attributes.children.insert(.element(newChild), at: idx)
                return
            }
        }
        attributes.children.append(.element(newChild))
    }

    private static func mergeCollectedAttributesIntoMeasure(
        measure: ChordOsmdXmlElement,
        collected: [String: ChordOsmdXmlElement]
    ) -> Bool {
        guard !collected.isEmpty else { return false }

        var changed = false
        var attributes = directChild(measure, localName: "attributes")
        if attributes == nil {
            let attrs = ChordOsmdXmlElement(name: "attributes")
            measure.children.insert(.element(attrs), at: newAttributesInsertIndex(in: measure))
            attributes = attrs
            changed = true
        }
        guard let attributesEl = attributes else { return changed }

        var existingKeys = Set<String>()
        for ch in attributesEl.children {
            if case let .element(el) = ch {
                existingKeys.insert(attributesChildKey(el))
            }
        }

        for (key, source) in collected where !existingKeys.contains(key) {
            insertAttributesChildInCanonicalOrder(
                attributes: attributesEl,
                newChild: deepCopyElement(source)
            )
            changed = true
        }
        return changed
