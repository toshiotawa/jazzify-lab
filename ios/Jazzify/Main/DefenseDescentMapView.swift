...
/// フレーズディフェンスの魔王城降下マップ（Web `DefenseDescentMap` 相当）。
...
private struct DefenseDescentMapContent: View {
...
private struct DefenseDescentBlockContent: View {
    ...
    var body: some View {
        ...
        let depthLabel = isEnglishCopy
            ? "FLOOR \(blockLayout.blockIndex + 1)"
            : "第\(blockLayout.blockIndex + 1)階層"
        ...
        SurvivalDescentBlockLantern(
            ...
            scale: scale,
            theme: theme,
            lit: !locked && (doorOpened || isFrontierBlock),
            dim: locked,
            side: .left,
        ...
        SurvivalDescentBlockHeaderPlate(
            label: isEnglishCopy ? blockLayout.labelEn : blockLayout.label,
        ...
        if let lastNode = blockLayout.nodes.last {
            if hasNextBlock {
                SurvivalDescentDoorView(
                    xPx: lastNode.x * scale + horizontalOffset,
