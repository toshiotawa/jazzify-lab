-- challenge_fantasy_tracksテーブルに転調設定の上書きカラムを追加
-- ミッションでファンタジーステージを選択した場合に
-- 元のステージ設定を上書きして使用できるようにする

-- 上書き用のリピート転調設定（NULLの場合は元の設定を使用）
ALTER TABLE public.challenge_fantasy_tracks 
...
-- カラムのコメント追加
COMMENT ON COLUMN public.challenge_fantasy_tracks.override_repeat_transposition_mode IS 
'リピート転調設定の上書き: NULLの場合は元のステージ設定を使用';

COMMENT ON COLUMN public.challenge_fantasy_tracks.override_start_key IS 
