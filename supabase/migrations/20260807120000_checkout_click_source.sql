-- checkout クリック元（初回のみ）+ paywall ファネル強化
ALTER TABLE user_milestones ADD COLUMN IF NOT EXISTS checkout_click_source text;
...
BEGIN
  ...
  IF p_milestone = 'checkout_click'
     AND p_source IS NOT NULL
     AND length(trim(p_source)) > 0 THEN
    UPDATE user_milestones
    SET checkout_click_source = COALESCE(checkout_click_source, trim(p_source)),
        updated_at = now()
    WHERE user_id = p_user_id;
  END IF;
END;
