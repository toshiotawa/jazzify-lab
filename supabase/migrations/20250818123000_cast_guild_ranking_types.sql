-- Ensure returned column types match RETURNS TABLE declarations for guild ranking RPCs

-- rpc_get_guild_ranking: cast count(*) to integer, sum(...) to bigint, rank() to integer
...
begin
  ...
  with monthly as (
    select g.id as guild_id, g.name, g.level,
           ...
           coalesce(sum(c.gained_xp)::bigint, 0::bigint) as monthly_xp
...
-- rpc_get_guild_monthly_ranks: cast sum(...) to bigint and rank() to integer
