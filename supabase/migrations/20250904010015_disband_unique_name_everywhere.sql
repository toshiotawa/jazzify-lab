...
-- Override: kick to ensure consistency (no name change here; included for completeness)
create or replace function public.rpc_guild_kick_member(p_guild_id uuid, p_user_id uuid)
returns void
language plpgsql security definer as $$
declare
  _uid uuid := auth.uid();
