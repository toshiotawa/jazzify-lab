-- Major II-V-I Bebop Lick (Short II-V 2 Bars): 5 stages x 12 keys x battle/precision
...
WHERE ls.lesson_id = l.id
  AND l.course_id = uuid_generate_v5('a0000000-0000-4000-8000-000000000002'::uuid, 'course-major-251-short-2bars');

DELETE FROM public.lessons WHERE course_id = uuid_generate_v5('a0000000-0000-4000-8000-000000000002'::uuid, 'course-major-251-short-2bars');
...
SELECT
  uuid_generate_v5('a0000000-0000-4000-8000-000000000002'::uuid, 'course-major-251-short-2bars'),
  'Major II-V-I Bebop Lick (Short II-V 2 Bars)',
  'Major II-V-I Bebop Lick (Short II-V 2 Bars)',
...
WITH ns AS (SELECT 'a0000000-0000-4000-8000-000000000002'::uuid AS id),
s(si, pf, pt, lm, tc, dur) AS (VALUES
  (1,1,4,33,72,66),(2,5,8,33,72,66),(3,9,12,33,78,66),
  (4,13,16,33,74,66),(5,17,20,33,74,66))
...
SELECT
  uuid_generate_v5(ns.id, 'm251-s2-lesson-st' || s.si),
  uuid_generate_v5(ns.id, 'course-major-251-short-2bars'),
  'Stage ' || s.si,
  'Stage ' || s.si,
...
SELECT
  uuid_generate_v5(ns.id, 'm251-s2-st' || s.si || '-' || k.slug || '-' || m.ms),
  'm251-s2-st' || s.si || '-' || k.slug || '-' || m.ms,
  'Short II-V 2Bars Stage ' || s.si || ' ' || k.kn || '（' || m.mja || '）',
  'Short II-V 2Bars Stage ' || s.si || ' ' || k.kn || ' (' || m.men || ')',
  'フレーズ ' || s.pf || '〜' || s.pt || '（全12キー・バトル / 精密）',
  'Phrases ' || s.pf || '-' || s.pt || ' (all 12 keys, battle / precision)',
  120, k.fifths, 4, 4, s.lm, 4,
  0, 600, 150,
...
SELECT
  uuid_generate_v5(ns.id, 'm251-s2-st' || s.si || '-' || k.slug || '-' || m.ms || '-ph0'),
  uuid_generate_v5(ns.id, 'm251-s2-st' || s.si || '-' || k.slug || '-' || m.ms),
  0,
  k.kn || '（' || m.mja || '）',
  k.kn || ' (' || m.men || ')',
  'https://jazzify-cdn.com/sozai/major-251-short-2bars/' || 'm251-s2-st' || s.si || '-' || k.slug || '-' || m.ms || '.musicxml?v=202608021200',
  'https://jazzify-cdn.com/sozai/major-251-short-2bars/m251-s2-st' || s.si || '-' || k.slug || '.mp3?v=202608021200',
...
SELECT
  uuid_generate_v5(ns.id, 'm251-s2-st' || s.si || '-' || k.slug || '-' || m.ms || '-lsong'),
  uuid_generate_v5(ns.id, 'm251-s2-lesson-st' || s.si),
  NULL::uuid,
  k.ord * 2 + m.ord,
