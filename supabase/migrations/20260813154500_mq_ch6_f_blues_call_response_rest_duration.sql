-- コール＆レスポンス譜の全休符 duration と小節埋め休符を直したので再キャッシュバスト
UPDATE public.ear_training_phrases p
SET music_xml_url = regexp_replace(p.music_xml_url, '\?v=[0-9]+$', '?v=202608131545')
...
SET
  script = replace(
    ...
    'guide-voice4-cue.musicxml?v=202608131545'
  )::jsonb,
  updated_at = now()
